...
The full proof for Result 1 is contained in Appendix \@ref(TWFE-proof), but here, we demonstrate the intuition discussed above. In the 2x2 case, the estimator, $\hat{\beta}_{TWFE}$, can be expressed:
...
This is similar to the typical DID estimator. However, by definition of $y^o_{it}$ this expression is only valid when $y^o_{i1}=0$. In the case where $y^o_{i1}=1$, it must be the case that $y^o_{i2}$ has been dropped. As a result, all pixels that contribute to the estimates of post-treatment effects must have a common pre-treatment outcome of 0. As a result, $\hat{\beta}_{TWFE}$ simplifies: 
