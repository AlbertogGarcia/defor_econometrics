...
Here we use analytical proofs to explore the properties of econometric models of nonrepeatable deforestation events in the simple two-group, two-period setting. First, in Section XX, we prove that TWFE models with pixel level fixed effects (Regression XX) do not estimate the $ATT$. In contrast, a more traditional DID (Regression XX) does recover an unbiased estimate of the $ATT$. Then, in Section XX, we explore how survival models can be used to generate unbiased estimates of a treatment effect in a difference in differences research design.  Finally, in Section XX we demonstrate that the occurrence of nonrepeatable events generates additional sample selection bias when XX.
...
## Alternate construction of $y^o_{ivt}$ {#retain-pixels}
...
We show in \@ref(keep-pixels-proof) that while the coefficient of a TWFE regression using $y^{alt}_{ivt}$ does not suffer from the \textit{nonrepeatable outcome, unit FE bias}, it estimates an alternate parameter from that of the $ATT$. Instead, the coefficient recovers an estimate of an intervention's average impact on cumulative deforestation across treated units. Identification of this treatment effect parameter requires the assumption that aggregate post-treatment clearing would have been the same in treatment and control areas without treatment. Consequently, if treatment and control areas have differing pre-treatment deforestation rates, this assumption implicitly requires a different deforestation rate among pixels still at risk across the two groups. This stems from the fact that the two groups will have a differing proportion of pixels at risk post-treatment. In addition, when reforestation timing is unaccounted for and deforestation is treated as an absorbing state, the interpretation of this parameter does not translate into the intervention's effect on deforested area (or forest cover if looking at $1 -$ deforested area).

The $ATT$ as defined in Section 2 represents an estimate of the impact of an intervention on the frequency of deforestation events (i.e. the decision to clear). Economists have typically focused their attention on this clearing decision, estimating the impact of a policy on the deforestation rate (flow) rather than the stock of ever-cleared forest. For the reasons discussed here, we use $y^o_{ivt}$ rather than $y^{alt}_{ivt}$ as the relevant outcome for the remainder of the paper. 
...
In response to the analytical results presented in Section XX, we draw upon Monte Carlo simulations to guide researchers towards empirical specifications that effectively address common sources of bias. First, in Section XX, we use our simulation model to illustrate the \textit{nonrepeatable outcome, unit FE bias} in an empirical setting. Then, in Section XX we show that researchers can easily address this bias by aggregating fixed effects or units of observation. Alternatively, Section XX shows that our proposed survival analysis-based estimator can also yield unbiased estimates of the $ATT$. However, all of these estimators may still face some bias due to sample selection, which we illustrate in Section XX. Next, in Section XX, we provide results guiding researchers towards the appropriate scale at which to aggregate their analyses. Finally, Section XX extends our analysis to settings with staggered exposure to a treatment.
...
It is possible for both $\beta_{FE,j}$ and $\beta_j$ from Regression 3 to suffer from bias similar to the \textit{nonrepeatable outcome, unit FE bias}. This occurs if an entire aggregated unit is deforested prior to the end of the panel. In practice, the bias is more likely to bleed into $\beta_{FE,j}$ or $\beta_j$ when the aggregated unit, $j$ has few pixels or when spatial autocorrelation is severe. Luckily, researchers can easily examine their dataset to determine whether this bias may be relevant.
