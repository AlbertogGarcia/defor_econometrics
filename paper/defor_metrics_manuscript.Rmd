...
<!-- These suggestions have the potential to improve the accuracy of econometric analyses of any binary, nonrepeatable outcomes.   -->
...
Intuition for this result stems from the use of individual unit fixed effects in the regression specification. By including individual unit fixed effects in TWFE regressions, researchers hope to control for local confounders, including pre-treatment differences in the outcome. However, when following common guidance to drop observations in the periods after the nonrepeatable event is first realized, these fixed effects do not behave as the researcher expects. Observations that realize the event (i.e., are deforested) in years prior to treatment are, by definition, not observed post-treatment. Because the event only happens once, estimation with individual unit fixed effects fails to remove time invariant heterogeneity at the unit level, making their inclusion meaningless. In the context of nonrepeatable outcomes, $\beta_{TWFE}$ is distinct from $\beta_{DID}$, even in the 2x2 case. 

<!-- Excerpt from Farbmacher: However, the logic that these transformations remove unit-specific time-invariant heterogeneity and therefore allow for consistent and unbiased estimation by least squares does not apply in a discrete-time hazard setting, in which an observation unit is observed only until that period in which the event of interest occurs. The impossibility of eliminating unit-specific unobserved heterogeneity by transforming the data becomes intuitive by recognizing that, in this setting, only a single outcome, that is, the length of the spell at risk, is observed for a unit, although the data seemingly exhibits a panel structure. -->
...
The use of $y^{alt}_{ivt}$ introduces its own concerns, however. In settings with nonrepeatable outcomes, unit $i$ is only effectively observed until $C_0$. If $y_{ivt} = 1$, unit $i$ has reached an absorbing state and for all periods in which $y^{alt}_{ivt}=1$, only one period of risk is actually observed. Therefore, any panel regression will not use observations on $i$ later than the period in which $i$ enters the absorbing state, because they are not informative about the transition to that state [@farbmacher_linear_2023]. In other words, any subsequent observations of $y^{alt}_{ivt}$ beyond $C_0$ do not contain any information about the data generating process of interest.

<!-- , because $P(y^{alt}_{ivt+s} = 1 | y^{alt}_{ivt} = 1)$ always holds for $s\geq 1$. -->
...
One important source of spatial autocorrelation in land use is the spatial area over which landowners or other decisionmakers exert their influence. Property level unobservables such as the preferences and resources of a landowner may drive significant variation in land use. If unaccounted for in an impact evaluation, these property level differences might affect both treatment effect estimates and coverage probabilities. 

To quantify the impact of property-level differences in the returns from forest clearing, we vary $\sigma_p$, the standard deviation of time-invariant property level disturbances in the DGP. The introduction of these perturbations introduces positive spatial autocorrelation between pixels from a common property. That is, pixels located within the same property share an increased or decreased propensity to be cleared. 

The introduction of $\sigma_p$ changes the performance of standard econometric models. For example, the pixel-level DID does not account for spatial autocorrelation in deforestation decisions. As property-level disturbances become larger, the pixel-level DID model's performance in terms of bias, RMSE, and coverage declines (Figure \@ref(fig:did-property)). 
...
\begin{align}
y^{alt}_{it} = \sum^t_{k = 1} y^{C}_{it}
\end{align}
...
Similarly, make the following analog common trends assumption, now relating to $y^{alt}_{it}$:

\begin{align}
 E[y^{alt}_{i2}(0) - y^{alt}_{i1}(0)| D_i = 1] = & E[y^{alt}_{i2}(0) - y^{alt}_{i1}(0)| D_i = 0] 
\end{align}

$ATT_{alt}$ can be rewritten in terms of $y^C_{it}$ based on equation 23 above.
...
Equation \@ref(eq:lat-ATT-int) now makes the interpretation of this alternative treatment effect parameter more clear. $ATT_{alt}$ is equal to the average effect of treatment on accumulated deforestation across all post-treatment years for treated units.

We can also rewrite the new common trends assumption, where it simplifies to the following:
\begin{align}
...
```{r keep-pixels, message=FALSE, out.width="100%", fig.cap="Distribution of DID and individual unit TWFE coefficient estimates. Distributions of DID and TWFE coefficients when keeping pixels appear in purple, since they are numerically equivalent.", fig.width = 8, fig.height = 5, fig.pos='H', fig.show='hold', fig.align='center'}
...
ggplot(data = dropkeep_coeff, aes(x = coeff, fill=model)) +
  ...
  geom_vline(xintercept = DID_keep_estimand, color = palette$dark, linetype = "dashed")+
  ...
  scale_x_continuous(limits = c(-0.025, DID_keep_estimand + 0.01),
                     breaks = c(-0.02, ATT, 0, 0.01, DID_keep_estimand),
                     labels = c("-0.02", "ATT (-0.01)", "0", "0.01", expression(psi))
  )+
  annotate("text", x = ATT - 0.003, y = 20, size = 5,
           parse = T, label = as.character(drop_beta))+
  annotate("text", x = 0.011, y = 50, size = 5,
