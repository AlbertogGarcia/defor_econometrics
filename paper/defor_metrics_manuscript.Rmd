---
title: "Estimating impacts on non-reversible outcomes: the case of forest conservation"
author: 
...
The full proof for Result 1 is contained in Appendix \@ref(TWFE-proof), but here, we demonstrate the intuition discussed above. In the 2x2 case, the estimator, $\hat{\beta}_{TWFE}$, can be expressed as:
...
In principle, it is possible for $\beta_{FE,j}$ from Regression 3 to suffer from bias similar to that of the individual unit TWFE regression. This occurs if an entire aggregated unit, $j$ is deforested prior to $t_0$. Figure \@ref(fig:varying-gridsize) in Section \@ref(gridsize-bias) shows bias in $\beta_{FE,j}$ as a function of grid size. When grid sizes are particularly small, $\beta_{FE,j}$ suffers from bias, since it is more likely that an entire grid cell becomes deforested early on. Because this is more likely to occur in grid cells with higher pre-treatment deforestation rates (the treatment group in our simulations), some of the \textit{nonrepeatable unit TWFE bias} leaks into $\beta_{FE,j}$ of the aggregated unit TWFE regression. In practice, this bias is more likely to arise when the aggregated unit, $j$ has few pixels or when spatial autocorrelation is severe. Luckily, researchers can examine their dataset to determine whether this bias may be relevant.
...
```{r keep-pixels, message=FALSE, out.width="100%", fig.cap="Distribution of DID and individual unit TWFE coefficient estimates. Distributions of DID and TWFE coefficients when keeping pixels appear in purple, since they are numerically equivalent. $\\psi$ denotes the estimand as derived in A.2.4.", fig.pos='H', fig.align='center'}
...
ggplot(data = dropkeep_coeff, aes(x = coeff, fill=model)) +
  ...
  scale_x_continuous(limits = c(-0.025, DID_keep_estimand + 0.01),
                     breaks = c(ATT, 0, DID_keep_estimand),
                     labels = c("ATT(-0.01)", "0", expression(psi)))
```
