...
In response to this nonrepeatability, @jones2015 suggest that deforested pixels should be dropped in the periods after they are first cleared. We, alongside most studies in this literature, follow this guidance in our baseline model, further modifying the definition of our binary deforestation outcome:
...
In response to the \textit{nonrepeatable outcome, unit FE bias} shown above, some researchers may consider an alternate construction of the outcome variable, whereby units for which the nonrepeatable event has occurred are left in the panel:
...
The use of $y^{alt}_{ivt}$ introduces its own concerns, however. In settings with nonrepeatable outcomes, unit $i$ is only effectively observed until $C_i$. If $y_{ivt} = 1$, unit $i$ has reached an absorbing state and for all periods in which $y^{alt}_{ivt}=1$, only one period of risk is actually observed. Therefore, any panel regression will not use observations on $i$ later than the period in which $i$ enters the absorbing state, because they are not informative about the transition to that state [@farbmacher_linear_2023]. In other words, any subsequent observations of $y^{alt}_{ivt}$ beyond $C_i$ do not contain any information about the data generating process of interest.
...
In principle, it is possible for $\beta_{FE,j}$ from Regression 3 to suffer from bias similar to that of the individual unit TWFE regression. This occurs if an entire aggregated unit, $j$ is deforested prior to $t_0$. Figure \@ref(fig:varying-gridsize) in \@ref(gridsize-bias) shows bias in $\beta_{FE,j}$ as a function of grid size. When grid sizes are particularly small, $\beta_{FE,j}$ suffers from bias, since it is more likely that an entire grid cell becomes deforested early on. Because this is more likely to occur in grid cells with higher pre-treatment deforestation rates (the treatment group in our simulations), some of the \textit{nonrepeatable outcome, unit FE bias} leaks into $\beta_{FE,j}$ of the aggregated unit TWFE regression. In practice, this bias is more likely to arise when the aggregated unit, $j$ has few pixels or when spatial autocorrelation is severe. Luckily, researchers can examine their dataset to determine whether this bias may be relevant.

Columns 5-7 show that regressions with more aggregated units of analysis (e.g. Regression 4) also yield unbiased estimates of the $ATT$. In these regressions, the outcome of interest is transformed from the individual binary occurrence of deforestation in pixel $i$ into the aggregated rate of deforestation occurring across a broader spatial unit $j$ in each time $t$. In our primary results this aggregate rate is calculated using equation \@ref(eq:deforrate). Once again, we see that in the absence of property level perturbations (i.e. $\sigma_p=0$ in the DGP), neither the bias nor RMSE of the estimates vary dramatically across different levels of aggregation. 

## Survival analysis

Survival analysis provides an appealing alternative to traditional linear estimators when studying nonrepeatable changes such as deforestation. The simple, single-regression DID framing of the Cox Proportional hazards model  (Regression 5), however, does not yield an unbiased estimate of the $ATT$ (Section \@ref(cox-bias)) under the typical common trends assumption (Assumption 1). Column 8 of Figure \@ref(fig:summary-fig) shows the bias associated with this model in our parameterization where the parallel trends assumption (Assumption 1) holds but the proportional trends assumption (Assumption 3) does not. In light this bias, we explore whether our proposed estimator, $\widehat{ATT_{Cox}}$, recovers relevant treatment effect parameters in the deforestation setting. We see in column 9 that $\widehat{ATT_{Cox}}$ does recover an unbiased estimate of the $ATT$.
