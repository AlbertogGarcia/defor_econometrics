...
Importantly, there is a distinction between observing a clearing event for an at-risk pixel and $y^C_{it}$. This is because $y^C_{it} = 0$ for all $t > C_i$. As such, the interpretability of $ATT_{alt}$ is affected. It is not recovering the effect for at-risk pixels and is highly influenced by earlier deforestation events. Further, this causes $ATT_{alt}$ to be mechanically influenced by the length of time a previously deforested pixel is left in the panel, a less than desirable property. 
