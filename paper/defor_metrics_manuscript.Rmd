...
<!-- Robert TODO: Revise this paragraph a bit to clarify the challenge we identify -->
...
The remainder of the paper proceeds as follows. In Section \@ref(setup), we introduce our empirical context, focusing on the nonrepeatability inherent to forest conservation impact evaluation. We also discuss common panel econometric approaches used in the conservation literature and in other fields that often encounter nonrepeatable outcome data. Section \@ref(methods) describes the setup for our analytical proofs and the Monte Carlo simulations we use to evaluate candidate econometric approaches in the context of a simulated conservation intervention. In Section \@ref(results-analytical), we present the results of our analytical proofs, including the identification of the \textit{Nonrepeatable unit fixed effects bias} associated with TWFE models incorporating individual unit fixed effects and the non-random sample selection that may stem from nonrepeatability inherent to these settings. We also introduce our newly developed survival estimator. Next, in Section \@ref(results-simulation), we present results from our Monte Carlo simulations that illustrate the biases mentioned previously along with proposed solutions. We then explore the advantages to matching analyses to the scale of the decisionmaking unit and discuss the interpretation of coefficients from analyses incorporating area weighting. Additionally, we simulate an intervention with staggered adoption to show that many recently developed DID estimators fail to recover the expected treatment effect parameter when pixels are used as the unit of analysis, similar TWFE regressions with individual unit fixed effects. Finally, Section \@ref(conclusion) concludes. 
...
## TWFE with nonrepeatable outcomes yields biased estimate of $ATT$ 
...
\textbf{Result 1:} (Nonrepeatable unit FE bias)
\begin{align}
...
Intuition for this result stems from the use of individual unit fixed effects in the regression specification. By including individual unit fixed effects in TWFE regressions, researchers hope to control for local confounders, including pre-treatment differences in the outcome. However, when following common guidance to drop observations in the periods after the nonrepeatable event is first realized, these fixed effects do not behave as the researcher expects. Observations that realize the event (i.e., are deforested) in years prior to treatment are, by definition, not observed for the entire panel. Because the event only happens once, estimation with individual unit fixed effects becomes meaningless. In fact, $\beta_{TWFE}$ differs from $\beta_{DID}$ in our context, even in the 2x2 case. 

<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### BELOW: ADDED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-->

The full proof for Result 1 is contained in \@ref(TWFE-proof), but here, we demonstrate the intuition discussed above. In the 2x2 case, the estimator, $\hat{\beta}_{TWFE}$, can be expressed as:

\begin{align*}
\hat{\beta}_{TWFE}&= \frac{1}{n_{i:D_i=1}}\sum_{i:D_i=1} y^o_{iv2} - \frac{1}{n_{i:D_i=1}}\sum_{i:D_i=1} y^o_{iv1} - (\frac{1}{n_{i:D_i=0}}\sum_{i:D_i=0} y^o_{iv2} - \frac{1}{n_{i:D_i=0}}\sum_{i:D_i=0} y^o_{iv1})
\end{align*}

This is similar to the typical DID estimator. However, by definition of $y^o_{ivt}$ this expression is only valid when $y^o_{iv1}=0$. In the case where $y^o_{iv1}=1$, it must be the case that $y^o_{iv2}$ has been dropped. As a result, all pixels that contribute to the estimates of post-treatment effects must have a common pre-treatment outcome of 0. As a result, $\hat{\beta}_{TWFE}$ simplifies: 

\begin{align*}
\hat{\beta}_{TWFE} &= \frac{1}{n_{i:D_i=1}}\sum_{i:D_i=1} y^o_{iv2} - \frac{1}{n_{i:D_i=0}}\sum_{i:D_i=0} y^o_{iv2}
\end{align*}

That is, $\hat{\beta}_{TWFE}$ is equal to a simple ex-post difference in means between treatment and control units, which extends to Result 1 under the relevant assumptions. 

Section \@ref(results-simulation) corroborates Result 1 through simulations and shows that the traditional DID does not suffer from similar concerns in the context of forest conservation. In addition, we show in Section \@ref(TWFE-equivalence) that $\hat{\beta}_{TWFE}$ is equivalent to the coefficient from a TWFE regression on a dataset where any pixel deforested in years prior to treatment is completely excluded from the dataset.

<!-- Robert TODO: Not 100% convinced this is the right place for this paragraph. Think this through. -->
Several of the papers listed in Table 1 combine matching with regression analysis. This involves applying regression analysis to a sample consisting of treated and matched control units [@ho_matching_2007; @blackman2013]. Pre-processing generally reduces the pre-treatment difference in observable characteristics between treatment and control groups, meaning that the bias detailed in equation 10 is likely to be reduced. However, even when bias is reduced, the individual unit TWFE regression is still undesirable. First, the matching algorithm often will not have perfectly balanced all of the observable covariates across treatment and control observations [@jones2015]. This means that the simple ex-post difference in means provided by the individual unit TWFE regression will have nonzero bias if pre-treatment deforestation rates are not perfectly balanced. In contrast, a DID approach that can control for these differences will not suffer from any remaining unbalance in pre-treatment deforestation rates. More fundamentally, combining matching with outcome regression moves researchers toward a double-robustness property, whereby they can trade off between accurately modeling treatment assignment and the outcome regression [e.g., @santanna_doubly_2020]. As such, given a set matching strategy, researchers should attempt to model the outcome regression as accurately as possible. In light of our findings, the individual unit TWFE model is unlikely to be the best choice to do so.

## Alternate construction of $y^o_{ivt}$

<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### SECTION HEAVILY UPDATED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-->

In response to the Nonrepeatable unit FE bias shown above, some researchers may consider an alternate construction of the outcome variable, whereby units for which the nonrepeatable event has occurred are left in the panel:
...
The use of $y^{alt}_{ivt}$ introduces its own concerns, however. In settings with nonrepeatable outcomes, unit $i$ is only effectively observed until $C_0$. When $y_{ivt} = 1$, unit $i$ has reached an absorbing state. Consequently, for $y^{alt}_{ivt}=1$, only one period of risk is observed for any unit $i$. In other words, any subsequent observations of $y^{alt}_{ivt}$ beyond $C_0$ do not contain any information about the data generating process of interest, because $P(y^{alt}_{ivt+s} = 1 | y^{alt}_{ivt} = 1)$ always holds for $s\geq 1$, irrespective of right-hand side variables. The number of periods for which unit $i$ is effectively observed is therefore not fixed but endogenous [@farbmacher_linear_2023]. 

We show in \@ref(keep-pixels-proof) that the use of $y^{alt}_{ivt}$ recovers an alternate parameter from that of the $ATT$. In the context of deforestation, identification of this treatment effect parameter requires the assumption that aggregate post-treatment clearing would have been the same in treatment and control areas without treatment. Consequently, if treatment and control areas have differing pre-treatment deforestation rates, this assumption implicitly requires a different deforestation rate among pixels still at risk across the two groups. This stems from the fact that the two groups will have a differing proportion of pixels at risk post-treatment. The $ATT$ as defined in Section 2 represents an estimate of the impact of an intervention on the frequency of deforestation events (i.e. the decision to clear). Economists have typically focused their attention on this clearing decision, estimating the impact of a policy on the deforestation rate (flow) rather than the stock of cleared forest. For the reasons discussed here, we use $y^o_{ivt}$ rather than $y^{alt}_{ivt}$ as the relevant outcome for the remainder of the paper. 

<!-- Under this interpretation, keeping deforested pixels in the panel beyond the first period in which it was deforested would incorrectly imply that it has actively been deforested in each subsequent time period when, in fact, no new deforestation event or clearing decision has occurred.  -->
...
## Non-random sample selection can generate bias in nonrepeatable settings {#selection-bias}
...
<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### BELOW: ADDED EXPLANATION IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-->

This bias is due to a difference between the expectation of the $y^o_{iv2}$s and $y_{iv2}$s. The terms containing $y^o_{iv2}$ are conditional on the pixel remaining forested after the first period. That is, $y^o_{iv2}$ is only observed when $y_{ivt} = 0$. If the $y^o_{ivt}$s are i.i.d., $E[y^o_{ivt}] = E[y_{ivt}]$. However, this may not be the case in reality. For example, pixels with extremely high returns to clearing are more likely to be cleared early on, meaning that in subsequent periods, these high return pixels are less likely to be present in the sample. Therefore, the distribution of the returns to clearing across the landscape may change between the first and second period. If this changes differently between the treatment and control group, non-random sample selection bias may arise.
...
Figure \@ref(fig:did-twfe) further shows that the TWFE and DID estimators are not numerically equivalent, even in our 2x2 context. We further examine this finding in \@ref(TWFE-equivalence), showing that the coefficient of interest from Regression 2 is numerically equivalent to that from the same regression on a dataset where all pixels deforested in the first period are excluded from the dataset completely (i.e., there is no deforestation in the dataset pre-treatment). This exercise explicitly demonstrates that the individual unit TWFE regression uses none of the pre-treatment variation in deforestation, which is crucial to DID approaches.
...
Alternate forms of aggregation may provide a useful way to avoid the issues associated with integrating TWFE models with binary outcomes when the outcome is nonrepeatable. Columns 2-4 of Figure \@ref(fig:summary-fig) show that in pixel level TWFE regressions in the form of Regression 3, $\beta_{FE,j}$ can be an unbiased estimator of the $ATT$. These regressions use grid, county, or property fixed effects rather than pixel fixed effects. We also see that, in the absence of property level perturbations (i.e. $\sigma_p=0$ in the DGP), all three models provide similar estimates and estimate distributions. 

<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### BELOW PARAGRAPH: ADDED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-->

In principle, it is possible for $\beta_{FE,j}$ from Regression 3 to suffer from bias similar to that of the individual unit TWFE regression. This occurs if an entire aggregated unit, $j$ is deforested prior to $t_0$. Figure \@ref(fig:varying-gridsize) in \@ref(gridsize-bias) shows bias in $\beta_{FE,j}$ as a function of grid size. When grid sizes are particularly small, $\beta_{FE,j}$ suffers from bias, since it is more likely that an entire grid cell becomes deforested early on. Because this is more likely to occur in grid cells with higher pre-treatment deforestation rates (the treatment group in our simulations), some of the \textit{nonrepeatable unit TWFE bias} leaks into $\beta_{FE,j}$ of the aggregated unit TWFE regression. In practice, this bias is more likely to arise when the aggregated unit, $j$ has few pixels or when spatial autocorrelation is severe. Luckily, researchers can examine their dataset to determine whether this bias may be relevant.
...
<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### BELOW 3 PARAGRAPHS: UPDATED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-->

Although our proposed, survival analysis-based approach to estimation yields a good estimate of the true $ATT$ in this simple setting, multiple considerations raise questions about the utility of this non-linear model in more complex settings. As we show in the following sections, survival analysis is likely to suffer from bias when the data generating process underpinning deforestation is influenced by unobservable characteristics of more aggregated spatial units such as the preferences of a property-owner. In Section \@ref(full-summary), we see that when property-level unobservables determine pixels' propensity to be cleared, the modified Cox model performs poorly relative to linear models that incorporate aggregated units of analysis, helping to account for spatial autocorrelation. It is straightforward to account for this in OLS, but factors such as incidental parameters problems and computational difficulty in non-linear models may complicate easy implementation in survival analysis [@lancaster_incidental_2000; @fernandez-val_2016]. 

The most crucial issue that survival analysis addresses is censoring [@turkson_handling_2021]. This occurs when the researcher has partial information about the subjects’ survival times but does not have access to precise event times. Many common forms of censoring are rarely a concern in the context of deforestation. In fact, remote sensing typically enables the creation of large, balanced panels. Further, the proposed strategy to drop pixels in the periods after they are first deforested successfully addresses nonrepeatability in deforestation events.  

While the above factors make both the Cox DID regression and $\widehat{ATT}_{Cox}$ less appealing in the deforestation context, they may be of more utility in settings where researchers have traditionally been drawn to survival analysis for reasons beyond the binary nature of the outcome. Censoring that never arises in the deforestation context such as random, double, independent, or interval censoring may lead a researcher to demand a survival approach  [@turkson_handling_2021]. When comparing the Cox DID regression and $\widehat{ATT}_{Cox}$, the determination of a plausible trends assumption is key. If the traditional common trends assumption is plausible, $\beta_{coxDID}$ from Regression 5, will not identify the desired treatment effect parameter.
...
Finally, we provide evidence suggesting that researchers should seek to align the structure of their econometric models to match the real-world units at which land use decisions are being made. For example, if unobservable, property-level characteristics are thought to be an important driver of deforestation, the inclusion of property-level fixed effects can improve the accuracy of model estimates and inference. We encourage researchers to have an understanding of the institutional context to help decide which level of aggregation might best map onto the scale of autocorrelation within a landscape. Methods to address autocorrelation exist [e.g., @geniaux_new_2018], but we are unaware of any work empirically estimating the scale at which it operates within a landscape. Ultimately, context plays a role in what is feasible, and researchers should make clear the limits to their impact evaluation strategy. 
...
### Keeping pixels in periods after they are first deforested as an alternative outcome {#keep-pixels-proof}
<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### SECTION ADDED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-->

#### Setting up the alternate $ATT$

Some researchers may consider an alternate outcome variable, where rather than dropping units from the panel, the outcome is imputed as a 1 when $t > C_i$. Define this alternate outcome, $y^{alt}_{it}$, as follows:
...
\begin{align}
y^{alt}_{it} = \sum^t_{i = 1} y^{C}_{it}
\end{align}
...
And similarly, make the following analog common trends assumption, now relating to $y^{alt}_{it}$:

\begin{align}
 E[y^{alt}_{i2}(0) - y^{alt}_{i1}(0)| D_i = 1] = & E[y^{alt}_{i2}(0) - y^{alt}_{i1}(0)| D_i = 0]
\end{align}

This common trends assumption can be written in terms of $y^C_{it}$, and therefore simplifies to the following:
\begin{align}
E[  y^{C}_{i2}(0) + y^{C}_{i1}(0) - y^{C}_{i1}(0)| D_i= 1] = & E[ y^{C}_{i2}(0) + y^{C}_{i1}(0) - y^{C}_{i1}(0)| D_i= 0] \notag\\
\Leftrightarrow & \notag \\
E[  y^{C}_{i2}(0) | D_i= 1] = & E[ y^{C}_{i2}(0)| D_i= 0] (\#eq:CTA-alt)
\end{align}

This shows that the implicit common trends assumption made when using $y^{alt}_{it}$ as an outcome is equivalent to assuming that the probability of clearing post-treatment for the control units is equivalent to what it would have been for the treatment units had they not received treatment.



#### Coefficient under retention of deforested pixels
...
By using the same approach as above, we can derive the estimator, $\hat{\beta}_{impute}$:
...
Now imposing the common trends assumption given by equation \@ref(eq:CTA-alt) and substituting for $E[ y^{C}_{i2}(0)| D_i= 0]$ in $\beta_{impute}$ yields:
\begin{align*}
...
<!-- Because we observe $y^{alt}_{it} = 1$ anytime after $C_i$, we can express the probability that $y^{alt}_{it} = 1$ as a function of the probability that $C_i$ occurred prior to time $t$. Then, the terms with $y^{alt}_{i2}$s are re-expressed.  -->
...
## Monte Carlo evidence individual unit DID and TWFE recover alternate parameter if pixels are retained
<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### ADDED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
```{r keep-pixels, message=FALSE, out.width="100%", fig.cap="Distribution of DID and individual unit TWFE coefficient estimates. Distributions of DID and TWFE coefficients when keeping pixels appear in purple, since they are numerically equivalent.", fig.pos='H', fig.align='center'}

keeps <- readRDS("results/results_pixKeep.rds")

dropkeep_coeff <- keeps %>%
  pivot_longer(cols = c(1, 2, 3), names_to = "model", values_to = "coeff")

DID_keep_estimand = dropkeep_coeff$DID_keep_estimand[1]
ATT = dropkeep_coeff$ATT[1]
  
drop_beta = expression(hat(beta)[DID])

keep_beta = expression(hat(beta)[impute])
  
ggplot(data = dropkeep_coeff, aes(x = coeff, fill=model)) +
    geom_density(alpha = .5) +
    guides(fill=guide_legend(title=NULL))+
    geom_vline(xintercept = ATT, color = palette$green, linetype = "dashed")+
    theme(plot.caption = element_text(hjust = 0.5))+
     labs(x= "Coefficient estimates", y = "")+
  xlim(-0.02, 0.122)+
    theme_minimal(base_size = 12)+
  scale_fill_manual(labels=c("DID dropping pixels", "TWFE w/o dropping pixels", "DID w/o dropping pixels"), values = c( palette$green, palette$red, palette$blue)
                    )+
  scale_x_continuous(limits = c(-0.025, DID_keep_estimand + 0.01),
                     breaks = c(ATT, 0),
                     labels = c("ATT(-0.01)", "0"))+
  annotate("text", x = ATT - 0.0023, y = 20, 
           parse = T, label = as.character(drop_beta))+
  annotate("text", x = 0.0185, y = 20, 
           parse = T, label = as.character(keep_beta))
```
...
## Specifications in the form of Regression 3 may suffer from bias if aggregated unit is too small {#gridsize-bias}
<!-- 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
### ADDED IN REVISIONS
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
