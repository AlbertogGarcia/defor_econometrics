---
title: "Impact evaluation with irreversible outcomes\n: the case of forest conservation"
author: 
...
- More examples beyond deforestation: educational attainment, mortality
...
- not an assignment, but a question I have:
In 2x2 setting where each period does consist of one year, TWFE is equal to exactly the difference in post-treatment means. However, when each period consists of more years (e.g., Table 3) the TWFE is slightly different than the ex-post difference in means (and the final period difference in means). I'm guessing this has to do with proportion of pixels still present in the panel in the different post-treatment years, but not sure how much we want to precisely hone in on that difference. Might become more clear if we adjust proof to account for more years. 


COMPLETED:
- tried to ground Monte Carlo setup in potential outcomes 
- added example panel into main text in order to avoid same confusion as reviewer 4 
...
Below, we present a simple example of panel data with this structure. Suppose we observe a pixelated map of nine pixels with structure similar to @hansen2013 and are interested in deforestation over four years, where treatment of some pixels begins in year 3. Following the above guidance, Table XX yields the panel dataset. Note that panel datasets with this structure still maintain variation pre- and post-treatment.
...
Intuition for this result stems from the use of point unit fixed effects in the regression specification. By including point fixed effects in TWFE regressions, researchers hope to control for local confounders, including pre-treatment differences in the outcome. However, when following common guidance to drop observations in the periods after the irreversible event is first realized, these fixed effects do not behave as the researcher expects. Observations that realize the event (i.e. are deforested) in years prior to treatment are by definition not observed for the entire panel. Implicitly, the pre-treatment outcomes get assigned as 0 for both the treatment and control groups. In section \@ref(results), we show that $\hat{\beta}_{TWFE}$ is equivalent to the coefficient from a TWFE regression on a dataset where any pixel deforested in the pre-treatment period is simply dropped from the dataset completely.
...
One can also use fixed effects at the level of an aggregated unit rather than the point to resolve bias associated with TWFE regressions with point unit fixed effects. In the deforestation context, aggregation is often done spatially, aggregating pixels into larger units such as a grid cell [e.g., @rico-straffon2022], property [e.g., @heilmayr2016], or larger administrative unit [e.g., @pfaff1999]. Point-level TWFE models with aggregated unit fixed effects are all in the form of Regression 4.
...
Thus far, we have shown that $\beta_{TWFE}$ is equal
