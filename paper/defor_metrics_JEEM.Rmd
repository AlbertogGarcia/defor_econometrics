...
Finally, we reflect on the econometric benefits that emerge when researchers are able to match their model structure to the relevant scale of decisionmaking. For example, in settings where individual landowners may have strong, heterogeneous preferences affecting land-use change, we show that incorporating property-level fixed effects can reduce bias and improve statistical coverage over alternate model structures.

<!-- 1-3 paragraphs contribution to literature -->
Our conclusions yield important insights for the community of environmental economists, conservation scientists and policymakers that wish to understand the causal impacts of conservation policies. Causal impact evaluation in nature conservation has emerged relatively recently [@borner_effectiveness_2020], and a wide array of papers has called for researchers to improve the rigor of their approaches in this space [@ferraro2019; @avelino2016; @baylis2016; @miteva2012]. Our analyses raise potential concerns about the conclusions in many past studies, while providing researchers with practical guidance to improve the reliability of future analyses.

Our research also contributes to an emerging literature proposing methodological improvements for the integration of remotely sensed data with econometric methods of causal inference [@jain2020; @gibson_lights_2021]. Several studies have begun to document measurement error in satellite-based measurements and explore its implications for econometric analysis [e.g., @proctor_parameter_2023]. For example, @alix-garcia_remotely_2022 address misclassification in the context of a remotely sensed binary forest cover outcome and propose a solution for unbiased causal inference. Our paper illustrates that, even in settings without non-classical measurement error, researchers may still recover biased estimates of treatment effects when using common remotely sensed, deforestation data products.

<!-- Roadmap -->

The remainder of the paper proceeds as follows. Section \@ref(setup) introduces our empirical context. This includes the irreversibility inherent to forest conservation impact evaluation and common panel econometric approaches in forest conservation impact evaluation and other literatures that often encounter binary, irreversible outcome data. Section \@ref(methods) describes our analytical setup and Monte Carlo approach to evaluating candidate approaches in the context of a simulated conservation intervention. Section \@ref(results-analytical) presents results from our analytical proofs such as the ``irreversible unit FE bias" associated with TWFE models incorporating unit fixed effects and the ``selection bias" that may result from irreversibility inherent to these settings. We also introduce our new survival estimator. In Section \@ref(results-simulation), we present results from our extended Monte Carlo simulations that illustrate the biases mentioned above along with relatively straightforward solutions. We then demonstrate the benefits to matching analyses to the scale of the decision-making unit. Lastly, we simulate an intervention with staggered adoption to show that many recently developed DID estimators fail to recover the expected treatment effect parameter, falling prey to similar issues as TWFE with unit fixed effects. Finally, Section \@ref(conclusion) concludes. 
...
In addition to aggregating fixed effects, researchers can also transform their data by aggregating multiple pixel-level observations into larger units of analysis:
...
The outcome variable, $z_{jt}$, denotes the event rate within unit $j$ in period $t$. Arguably the most commonly used formula in the deforestation literature to calculate $z_{jt}$, the deforestation rate in unit $j$ in time $t$, uses the share of unit $j$ with forest cover and its lag [e.g. @carlson2018; @busch2015]:
\begin{align} 
