...
- clarify parameter of interest in setup section (deforestation rate i.e. flow of deforestation clearing, as opposed to area deforested)
-	Clarify that there are multiple years within both pre- and post-treatment periods, both in proof notation and monte carlo description
- add Pfaff et al. (2014, 2015a,b) and Robalino et al. (2015) to table?
- Adjust language away from "pixel" in proofs and intro wording
- More examples beyond deforestation: educational attainment?
- More intuition as to why TWFE fails in TWFE section
- Better labels on specification charts
- Discuss when keeping points may be useful - other settings beyond deforestation (i.e., poverty)
- More examples beyond deforestation: educational attainment
...
# Abstract {-}
Economists and conservation scientists are increasingly measuring treatment effects by applying quasiexperimental impact evaluation to binary, irreversible panel data. However, these applications come with new challenges. Using Monte Carlo simulations and analytical proofs, we demonstrate that many commonly employed panel econometric models are biased when applied to outcomes with this data structure such as pixel-level deforestation data. The significance, magnitude and even direction of estimated effects from many studies on conservation impacts are likely incorrect, threatening to undermine the evidence base that underpins conservation policy adoption and design. We develop guidance for the design of econometric models in this data setting, much of which is easily implemented in the conservation context. 

<!-- # Introduction -->
<!-- # Setup -->
<!-- # Two-way fixed effects with point fixed effects -->
...
Policymakers often need to understand the causal impacts of policy interventions that seek to avoid or increase the occurence of binary, irreversible events including mortality [e.g., @friedman], technology adoption [e.g., @bollinger], reincarceration [e.g., @mastrobuoni], and deforestation [e.g., XX]. Do anti-malaria initiatives reduce infant mortality? Are protected areas effective at reducing deforestation? Panel econometric methods such as difference-in-differences (DID) and two-way fixed effects (TWFE) regression are among the most popular approaches researchers adopt to conduct impact evaluation with observational data [@santanna_2018; @dechaisemartin_2020], and this extends to contexts with binary, irreversible outcomes. To date, insufficient attention has been paid to how such outcome data may affect the performance of popular panel econometric tools.

One panel setting in which this binary, irreversible structure is present is the case of forest conservation. A proliferation of studies detailing the impacts of conservation interventions on land use change has been enabled, in part, by the increasing prevalence of remotely sensed datasets detailing forest cover outcomes through time [@blackman2013]. For example, NASA's Landsat archive can be used to generate consistent information on land use spanning the entirety of the world since the 1970s [@hansen2012]. As a result, a scientist hoping to quantify the impacts of a land use policy adopted decades ago can assemble data for treated and control units that span both pre- and post-implementation periods [@jain2020]. 

Deforestation is often measured using data with a similar structure to the Global Forest Change product produced by @hansen2013. These data yield binary observations detailing the first year in which each 30 by 30m pixel was deforested. Most studies that evaluate the effectiveness of forest conservation policies use these remotely sensed pixels as the unit of observation [@borner2020]. Importantly, the data are unable to detect repeated deforestation events in the same location, so when converted to a panel structure, researchers have no information about a pixel's status beyond the year it is first cleared. In response, it has often been advised to drop pixels in the years after they are first cleared [@jones2015; @alix-garcia2017]. This yields an unbalanced panel dataset of binary, irreversible deforestation events.

While randomized experiments are the gold standard for the identification of causal relationships [@edwards2020; @jayachandran2017], conservation often poses questions that are prohibitively expensive, unethical or impossible to pursue through experimentation. In such settings, a growing portfolio of statistical techniques enable researchers to draw causal conclusions using observational data [@larsen2019; @bayis2016; @ferraro2014; @miteva2012]. Increasingly, these econometric approaches to impact evaluation are being used to disentangle the causal relationships that underpin conservation decisionmaking [@butsic2017; @baylis2016; @williams2020]. In particular, DID and TWFE regressions have become widely used, because they account for the non-random assignment of interventions that often confound identification of causal relationships in conservation. For example, a low rate of deforestation within a remote protected area may reflect the protected area's effectiveness, or it may be indicative of the remote location's poor suitability for agricultural development [@andam2008; @pfaff2009]. With DID and TWFE methods, a researcher may still be able to make claims about an intervention's impact, even if control units have a different deforestation rate prior to its implementation.  

Here, we use a combination of analytical proofs and Monte Carlo simulations to demonstrate that many econometric analyses are likely biased when applied to panel data with this structure- significance, magnitude and even direction of estimated effects might be incorrect. While we focus on deforestation, many of our results apply to any setting with binary, irreversible data. The resulting biases arise even when researchers follow common guidance to adopt "rigorous" research designs with valid counterfactuals [@blackman2013; @jones2015]. 

Our main result shows that TWFE regressions with point unit fixed effects do not identify the desired treatment effect parameter when applied to panel datasets with binary, irreversible outcomes. This is the case even when typical common trends assumptions hold and in the absence of general treatment effect heterogeneity. It has been widely stated that the standard DID estimator is numerically equivalent to the linear TWFE estimator in the two-group/two-period (2x2) case, where treatment is administered to only some units in the second period [@imai2021]. However, when applied to binary, irreversible panel data with the structure we describe, we demonstrate that the TWFE estimator is distinct from the standard DID estimator even in the 2x2 setting. We show that the coefficient of interest in TWFE specifications instead recovers an ex-post difference in deforestation rates between treatment and control areas. This is particularly worrisome in the context of forest conservation, where interventions often target areas with high deforestation rates (e.g., Brazil's blacklisted Priority Municipalities) or low opportunity costs (e.g., protected areas). Papers published in both conservation science and economics journals frequently use this problematic specification to recover treatment effect estimates. We also show that this concern extends to many recently developed DID estimators that estimate treatment effects in staggered adoption settings [e.g., @callaway_did_2020; @borusyak2021; @gardner2021].

To help guide future impact evaluations, we identify multiple ways in which this bias can be reduced or even eliminated. In the land use context, one easily implemented solution is to aggregate the binary pixels spatially. Both pixel-level TWFE specifications with spatially aggregated unit fixed effects and TWFE specifications with spatially aggregated units of analysis recover the expected treatment effect parameter in 2x2 settings. We also present a new survival-based DID estimator that relies on the common trends assumption typically invoked in linear models. This new estimator may be most beneficial in settings in which aggregation is not desirable for whatever reason. We then explore non-random selection that arises due to irreversibility in the deforestation setting and how this feature of the data may lead to bias. Finally, we reflect on the econometric benefits that emerge when researchers are able to match their model structure to the relevant scale of the deforestation process. 

# Setup

We focus on the case in which a researcher would like to quantify the impact an intervention has had on deforestation rates. We assume that the intervention has clearly defined boundaries (e.g., a protected area, certified concession, or indigenous territory), and that the researcher has access to spatially explicit observations of forest cover and forest loss spanning multiple years over the periods before and after the intervention was adopted. This general setting describes a broad array of studies that apply panel methods to remotely sensed data. Table \@ref(tab:table-lit) shows an unsystematic review of impact evaluation studies using data accessed as pixelated binary deforestation.
...
Below, we present a simple example of panel data following this guidance in the 2x2 setting. Suppose we observe data on nine pixels with structure similar to @hansen2013 and are interested in deforestation over six years, where treatment of some pixels begins in year 4. Note that panel datasets with this structure still maintain variation across groups, pre and post-treatment. 

```{r tile-fig, echo = FALSE, warnings = FALSE, results = FALSE, message=FALSE, fig.cap="Example pixelated deforestation data, indicating year of deforestation.",  fig.width = 3, fig.height = 3, out.width="100%"}
# Setup the data
m <- matrix(c("Pixel 1:\nnever\ncleared","Pixel 2:\ncleared in\nyear 2","Pixel 3:\ncleared in\nyear 4","Pixel 4:\ncleared in\nyear 1","Pixel 5:\ncleared in\nyear 6","Pixel 6:\ncleared in\nyear 3","Pixel 7:\ncleared in\nyear 5","Pixel 8:\nnever\ncleared", "Pixel 9:\ncleared in\nyear 2"), nrow=3, ncol=3, byrow = T)
...
Despite widespread use of pixel level analyses of deforestation, the application of TWFE models to a binary, irreversible process yields a biased estimate of the $ATT$. Specifically, we show that in the 2x2 case, the coefficient of interest from the TWFE model $(\beta_{TWFE})$ estimates the post-treatment difference in outcomes (single difference), rather than the desired $ATT$ (full proof contained in Appendix \@ref(TWFE-proof)). 
...
We now provide intuition behind this result, which stems from the use of point unit fixed effects in the regression specification. By including point fixed effects in TWFE regressions, researchers hope to control for local confounders, including pre-treatment differences in the outcome. However, when following common guidance to drop pixels in the periods after they are first deforested, these fixed effects do not behave as the researcher expects. Pixels that are deforested in years prior to treatment are by definition not observed for the entire panel. Implicitly, the pre-treatment deforestation rates get assigned as 0 for both the treatment and control groups. In section XX, we show that $\hat{\beta}_{TWFE}$ is equivalent to the coefficient from a regression on a dataset where all pixels deforested in the pre-treatment period are simply dropped from the dataset completely.
...
## Specifications utilizing spatial aggregation

### Spatially aggregated unit fixed effects

One can also use fixed effects at the level of an aggregated unit rather than the point to resolve bias associated with TWFE regressions with point unit fixed effects. In the deforestation context, aggregation is often done spatially, aggregating pixels into larger units such as a grid cell [], property [], or larger administrative unit []. Pixel-level TWFE models with spatially aggregated unit fixed effects are all in the form of Regression 4.
...
\end{align*}
, where $\lambda_t$ denotes year fixed effects and $\gamma_j$ denotes fixed effects at the level of a spatially aggregated unit. If $k$ differs from the level of treatment assignment, one must also include a treatment group indicator or fixed effects at the level of the unit at which treatment is assigned. 

### Spatially aggregated units of analysis

Another potential solution to the bias associated with TWFE models is for researchers to aggregate multiple pixel level observations into larger units of analysis. The researcher must now calculate the deforestation rate in each time period within the relevant unit. 

\textbf{Regression 5:} (TWFE regression with spatially aggregated unit of analysis)
...
\end{align*}
, where $\lambda_t$ denotes year fixed effects and $\gamma_j$ denotes fixed effects at the level of a spatially aggregated unit. Regression 5 differs from Regression 4 in both the treatment variable, $D_j$ and the outcome variable, $z_{jt}$. The treatment variable $D_j = \frac{1}{N_j}\sum_{i=1}^{N_j}D_i$, is the average treatment value amongst all pixels in unit $j$. The outcome variable, $z_{jt}$, denotes the deforestation rate within unit $j$ in period $t$. Arguably the most commonly used formula in the literature to calculate $z_{jt}$ uses the share of unit $j$ with forest cover and its lag [e.g. @carlson2018; @busch2015]:
\begin{align} 
z_{jt} &= \frac{F_{j,t-1} - F_{j,t}}{F_{j,t-1}} (\#eq:deforrate)
\end{align}
, where $F_{j,t}$ and $F_{j,t-1}$ are the share of spatial unit $j$ with forest cover at times $t$ and $t-1$, respectively. Note that in these regressions, the level of unit fixed effects matches the unit of analysis. 
...
Each of our simulated landscapes consists of administrative units that are either untreated $(D_i=0)$ or are assigned to a conservation treatment $(D_i=1)$. We observe deforestation in two, even-length periods, a pre-treatment $(t < t_{0})$ and a post-treatment $(t \ge t_{0})$ period. Each period consists of multiple years, ensuring that there is variation in deforestation in each year of the panel across both groups. 
...
The potential outcomes for the latent variable, $y^*_{ivt}$, are then as follows: 
\begin{align}
...
\begin{align}
y^*_{ivt}(1) =&\beta_0 + \beta_1 D_i +(\beta_{2,1} + \beta_3 )D_i \mathbbm{1}\{  t \geq t_0  \} +
 \alpha_i + u_{it} + \rho_{v}
\end{align}
