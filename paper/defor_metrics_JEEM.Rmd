...
- keep figures 3 and 4 consistent
- typo in "imputation bias section"
- Consistent use of "point" or "pixel" in proofs and early sections
- Monte Carlo description: number of iterations, landscape generatino, other descriptives
...
Our main result shows that TWFE regressions with individual unit fixed effects do not identify the desired treatment effect parameter when applied to panel datasets with binary, irreversible outcomes. This is the case even when typical common trends assumptions hold and in the absence of general treatment effect heterogeneity. It has been widely stated that the standard DID estimator is numerically equivalent to the linear TWFE estimator in the case of two-periods (each of which can consist of multiple years) and two-groups (2x2), where treatment is administered to only some units in the second period [@imai_kim_2021; @goodman-bacon2021]. However, when applied to binary, irreversible panel data with the structure we describe, we demonstrate that the TWFE estimator is distinct from the standard DID estimator. To our knowledge, this is the first documented case where the TWFE and DID estimators are numerically distinct in the two-period, two-group context. We show that the coefficient of interest in TWFE specifications instead recovers an ex-post difference in deforestation rates between treatment and control areas. This is particularly worrisome in the context of forest conservation, where interventions often target areas with high deforestation rates (e.g., Brazil's blacklisted Priority Municipalities) or low opportunity costs (e.g., protected areas). Papers published in both conservation science and economics journals frequently use this problematic specification to recover treatment effect estimates. This concern also extends to many recently developed DID estimators that estimate treatment effects in staggered adoption settings [e.g., @callaway2020; @borusyak2021; @gardner2021], meaning that researchers cannot simply adopt a different estimator to avoid the issue. 
