...
Survival analysis has emerged as a common approach to modeling irreversible events [@emmert-streib2019]. It used frequently to model events such as mortality [XX] and recidivism [@luallen_recidivism_2017; @mastrobuoni], but can also be appliedd to deforestation contexts. Survival models, such as the Cox Proportional Hazards model, quantify how covariates relate to changes in the length of time that a unit remains in a sample. In the case of deforestation, survival analyses can be used to explore how policy adoption changes the duration that treated, forested pixels survive until they are first cleared. 
