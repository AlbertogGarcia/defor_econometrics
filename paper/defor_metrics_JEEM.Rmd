...
We contribute to the literatures in environmental economics and conservation science seeking to rigorously estimate the impacts of conservation policy. 
...
Although this paper focuses upon impact evaluation of forest conservation policies, our findings are relevant to a wider audience. Specifically, these results apply to diverse settings in which the outcome of interest represents an irreversible, binary event. Such events include mortality [e.g., @friedman], technology adoption [e.g., @bollinger], and reincarceration [e.g., @mastrobuoni]. We identify several studies examining events such as mortality, recidivism, and technology adoption that fall prey to the same concerns we identify in this paper. 
...
```{r selection-fig, echo = FALSE, warnings = FALSE, results = FALSE, message=FALSE, fig.cap="bias, distribution, and coverage of all models with clustered standard errors.",  fig.width = 9, fig.height = 8, out.width="100%"}
...
Using the potential outcomes framework, we consider the potential outcomes for each observation as $y_{i,t}(D_i)$, where $D_i$ denotes the treatment status of unit $i$ in time $t$. In this notation and in the 2x2 case, the $ATT$ can be defined as

$$ATT = E[y_{i,2}(1)| D_i = 1] - E[y_{i,2}(0)| D_i = 1] $$
