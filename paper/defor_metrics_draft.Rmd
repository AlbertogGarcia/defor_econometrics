...
<!--
Formatting TODO:
Probably best to shift to embedded references rather than writing out section references - some sections were incorrectly referenced, and I've probably made an even bigger mess of things given my reorganization.
-->
...
<!--
TODO: The above paragraph assumes SUTVA. Maybe need to acknowledge that that's an assumption we're making at some point in the paper? Maybe we could add a short section to Monte Carlo description to note that we're imposing parallel trends and SUTVA to simplify the simulations?
-->
...
<!-- 
TODO: 
1. For the reader to be able to more identify the studies that fall prey to our main critique, do we need to add another column that describes the level unit fixed effects?
2. For "regression" panel methods - what does this mean? Aren't all of these studies using regression? Are these cross-sectional rather than panel models?
...
```{r counterfactual, echo=FALSE, fig.cap="This figure shows deforestation that would have ocurred in the absence of the conservation intervention at the same point in time, after implementation of the conservation intervention. Note that no counterfactual deforestation occurs in the control area, since no intervention ocurred.", out.width = "75%"}
knitr::include_graphics("figs/lscape.jpg")
```



<!-- 
TODO: I've only done a really rough job bringing these sections together. Ideally, we'd still:
1. Smooth integration between the previous paragraph and what's to come
2. Provide more definitions for the notation. For example, y_it wasn't defined until after this section.
3. Ensure we're not creating conflicting notation between this section and sections 3 and 4.1. Ideally all notation is consistent and builds upon previous sections.
4. Remove Figure 2? If we're headed towards REEP, might be better to add more complete mathematical notation and remove the simple visual describing DiD. 
5. Move some of the end of this section (e.g. reference to appendix 7.2) back into section 3? X was here
-->

As with the Global Forest Change product, researchers often observe maps depicting pixel-level, binary deforestation, similar to what is depicted in Figure \@ref(fig:counterfactual). When converted to a panel dataset, the researcher observes the year in which a pixel is converted from forest to non-forest, and importantly, deforested pixels do not revert back to forest cover over the course of the study period. Thus, repeated deforestation events cannot be detected in the same location. In order to correct for this irreversibility, it has been advised to drop deforested pixels in the periods after they first become deforested [@jones2015; @alix-garcia2017]. In the literature, the outcome variable describing pixel $i$, in property $v$, in time $t$ is often defined as follows:
\begin{align*}
 y_{ivt} = \begin{cases}
...
Analyses at the pixel level are prevalent in the literature, as seen in Table \@ref(tab:table-lit). Further, it has often been promoted as the preferred unit of analysis in many cases. Ultimately, the researcher seeks to estimate the $ATT$, which estimates the average impact of the conservation intervention on deforestation rate in pixels that receive it relative to the counterfactual scenario without the intervention. Let $y_{ivt}(1)$ and $y_{ivt}(0)$ denote the potential outcomes of pixel $i$ in property $v$ in time $t$ with and without the treatment, respectively. The $ATT$ is then the average effect of the conservation intervention on treated pixels and can be expressed: 
...
<!-- 
TODO: By defining the ATT and y_it above, I think we could introduce simple notation illustrating how the DID and TWFE estimate the ATT.
-->
...
We also evaluate the DID and TWFE models under the following stable unit value treatment assumption (SUTVA)

\textbf{Assumption 2:} (SUTVA)
\begin{align*}
\forall d \in \{ 0,1\}: \text{ if }D_i=d, \text{ then }y_{ivt}(d)=y_{ivt}
\end{align*}
...
\begin{align}
y_{it} = \alpha_0 + \alpha_1 D_i + \alpha_2 \mathbb{1} \{ t\geq t_0\}+ \beta_{DID} \text{ x } D_i \mathbb{1} \{ t\geq t_0\}  + \epsilon_{it}
\end{align}

The typical DID regression model includes a dummy variable equal to one for units in the treatment group, a dummy variable equal to one for observations in the period after the intervention, and their interaction. Conceptually, the DID estimator calculates the treatment effect as the difference between the differences of the treated and untreated observations before and after treatment [@butsic2017]. 
\begin{align*}
\beta_{DID} = E[y_{it} |  t\geq t_0, D_i=1] - E[y_{it} |  t<t_0, D_i=1] - (E[y_{it} |  t\geq t_0, D_i=0] - E[y_{it} |  t<t_0, D_i=0] )\\
\end{align*}
...
<!-- Figure \@ref(fig:twoperiod-deforestation) displays the intuition behind the DID estimator in the context of our simulated conservation intervention, given the deforestation rate trends in the intervention and untreated areas along with the unobservable counterfactual of what would have happened if the conservation intervention had not been adopted. -->

<!-- ```{r twoperiod-deforestation, fig.cap="The DID estimator (estimated ATT) amounts to the difference in pre-treatment deforestation rates (pre-treatment difference) minus the post-treatment difference in deforestation rates (post-treatment difference) between units with and without the intervention.", fig.show='hold', fig.pos = 'H', out.width = "75%"} -->
<!-- knitr::include_graphics(c("figs/did_trends.png")) -->

<!-- #knitr::include_graphics(c("figs/twoperiod_landscape.png","figs/did_trends.png")) -->
<!-- ``` -->

Often times, however, the researcher wants to estimate the $ATT$ in a setting that is not consistent with the two-group, two-period case covered by DID models. TWFE regression models are often used to apply DID methods to multiple groups or treatment periods. This generally amounts to estimating a regression that controls for unit and time fixed effects. Intuitively, this can be thought of as including a dummy variable for each unit of analysis and each time period. The fixed effects account for any unobservable confounding variables that may vary across units or through time. 
...
We follow [@kerr; @pfaff2004a] and model these binary deforestation events as a function of each pixel's unobservable value along a continous, latent variable ($y^*_{ivt}$) indicating the return to clearing pixel $i$, in property $v$, in year $t$. The latent variable is defined:
\begin{align*}
...
<!--
TODO: I think the equation above indicates that b3 and b2,1 are multiplied against the same term? Is that a typo?
-->
...
TWFE models have risen to prominence due to their flexibility in applying DID methods to settings with multiple groups and variation in treatment timing, however, we have shown that TWFE models are not a viable approach to estimate the $ATT$ with pixel fixed effects in deforestation impact evaluations. Column 1 of Figure \@ref(fig:summary-fig) shows the bias associated with the problematic regression described by equation (1). In our guiding example, the ex-post single difference is 0.02 (the $ATT$ plus the post-treatment group difference in deforestation rates), when the true $ATT$ is equal to -0.01. This means that a positive bias of 0.03 results from the use of this regression model.

However, we show that multiple alternate model specifications enable researchers to generate unbiased estimates of the $ATT$. We describe three straightforward solutions to this challenge below.

## Traditional difference-in-differences model

In the two-group, two-period case the typical DID model (Regression 1) is an unbiased estimator of the $ATT$, as shown in column 2 of Figure \@ref(fig:summary-fig). However, researchers often want to use TWFE models because of their flexibility in situations that do not fall under the typical DID setup. Therefore, researchers should be aware of the trade-offs using aggregated units of analysis and fixed effects when using TWFE models for deforestation impact evaluation. 

<!--
```{r pixpics, fig.cap="This figure demonstrates the spatial extent and scale of common units of analysis.", fig.show='hold', fig.pos = 'H', out.width = "30%"}
...
<!--
TODO: Need to add a bit more detail on this aggregation process, and how deforestation rates are calculated. Currently I worry the "XX-level" analyses are a bit mysterious to a reader
-->

Aggregated units of analysis overcome the issues that we have identified with respect to pixel-level TWFE regressions. For simplicity, we assume the researcher can choose between three levels at which to aggregate the data: grid cell, county, and property. Grid cells are uniform grids layered over the study area and may have a treatment value between 0 and 1 following aggregation of pixels. Counties are heterogeneous administrative units at which we now assign the treatment. Lastly, properties are smaller administrative units that lie within one or more counties. The following results apply to both DID and TWFE regression models, as both are equivalent in the two-period, two-group example when pixels are not dropped. The researcher does not have to drop pixels in the periods after they first become deforested when using aggregated units of analysis, but they must calculate the deforestaion rate in each time period. While we detail the tradeoffs of various deforestation rate calculations in the appendix (), the following results are based on one common formula to calculate deforestation rates. 
\begin{align*}
rate_{jt} &= \frac{F_{j,t-1} - F_{jt}}{F_{j,t-1}}
\end{align*}
, where $F_{jt}$ and $F_{j,t-1}$ are the forest cover in unit $j$ at times $t$ and $t-1$, respectively. Unit $j$ represents one of the three aggregated units, the grid cell (g), county (c), or property (v). This calculation is used consistently in the literature [e.g. @carlson2018; @busch2015a], and is arguably the most widely used formula.

\textbf{Regression 3:} (grid/county/property level regression)
...
\begin{align}
rate_{jt} = \alpha + \beta_{j} \text{ x } D_j \mathbb{1} \{ t\geq t_0\} + \gamma_t + \lambda_j + \epsilon_{jt}
\end{align}
, where $\gamma_t$ denotes year fixed effects and $\lambda_j$ denotes one of grid (g), county (c), or property (v) fixed effects. The treatment variable $D_j = \frac{1}{N_j}\sum_{i=1}^{N_j}D_i$, is the average treatment value amongst all pixels in unit $j$.

We find little evidence that any one aggregated unit of analysis is consistently preferrable in terms of bias. Columns 3-5 of Figure \@ref(fig:summary-fig) show that the bias of the estimates is not critically different depending on level of aggregation, however, the distributions are varied.  In particular, the property-level model had the highest RMSE of unbiased specifications due to the heterogeneity in property sizes across the landscape. 

<!-- ```{r agg_units, fig.cap="fig caption", fig.show='hold', fig.pos = 'H'} -->
<!-- knitr::include_graphics("figs/aggregated_ua.png") -->
<!-- ``` -->

```{r summary-fig, fig.cap="bias, distribution, and coverage of all models with standard errors clustered at the level of the model fixed effects. The pixel level DID is clustered at the pixel level. ",  out.width="100%"}
knitr::include_graphics("figs/summary_fig1.png")
```

## Aggregated fixed effects

One can also use fixed effects at the level of an aggregated unit rather than the pixel level to resolve bias associated with pixel-level TWFE regressions with pixel fixed effects. These regression models are all in the form of Regression 4, with varying levels of $j=\{g,c,v \}$. 

\textbf{Regression 4:} (pixel level TWFE regression with aggregated fixed effects)
...
\begin{align}
y_{it} = \alpha + \beta_{FE,j} \text{ x } D_i \mathbb{1} \{ t\geq t_0\} + \gamma_t + \lambda_j + \epsilon_{it}
\end{align}
, where $\gamma_t$ denotes year fixed effects and $\lambda_j$ denotes one of grid (g), county (c), or property (v) fixed effects.

Columns 6-8 of Figure \@ref(fig:summary-fig) show that pixel level TWFE regressions are unbiased estimators of the $ATT$ when grid, county, or property fixed effects are used rather than pixel fixed effects. We also see that, in the absence of property level perturbations (i.e. $\sigma_p=0$ in the DGP), all three models provide nearly identical estimates and estimate distributions. The RMSE of these models is slightly smaller than that of the corresponding models with aggregated units of analysis. 
...
# The value of linking model structure to the scale of decision-making

We now demonstrate the importance of connecting the econometric model to the process by which land use change occurs on the ground. Table \@ref(tab:table-lit) shows that researchers often use a binary point or pixel as the unit of analysis. While this may sometimes be the preferred unit, it may be undesireable if the land use change process operates at a larger scale. Property level unobservables may impact both treatment effect estimates and coverage probabilities. This is likely to be a factor when land use decisions are made at the property level, or the intervention seeks to alter underlying landowner incentives. In our Monte Carlo simulations, we vary the standard deviation of the time-invariant property level disturbances in the initial DGP, $\sigma_p$, in order to account for these unobservables. 

<!-- Implications for bias -->

The introduction of property level unobservables changes the relative performance of each level of aggregation. Figure \@ref(fig:property-uoa) demonstrates that models incorporating the property into the analysis, whether with aggregated units of analysis or fixed effects, perform relatively better than their counterparts in terms of bias when the standard error governing the property level disturbances, $\sigma_p$, increases. The left panel of Figure \@ref(fig:property-uoa) demonstrates that the property-level specification suffers from less bias that models with any other unit of analysis when $\sigma_p$ is non-zero. The pixel level DID suffers from the highest levels of bias relative to alternative units of analysis. 

The right panel of Figure \@ref(fig:property-uoa) demonstrates a similar takeaway in terms of models that aggregate fixed effects. We see that models with grid, property, or county fixed effects perform similarly in terms of bias when there are no property level disturbances. However, as $\sigma_p$ increases, the model with property fixed effects suffers from less bias than the others. These findings indicate that incorporating the relevant decision making unit into the econometric specification can help to reduce estimate bias. 
...
Also, would it make sense to add DID to the FE panel? This is really just a treated / non-treated FE.
-->
...
As property level unobservables play a larger role, the treatment of standard errors also becomes more important. Standard error clustering is used by nearly every study in the literature for inference. The clustering problem is caused by the presence of a common unobserved random shock at the group level that will lead to correlation between all observations within a group [@hansen2007]. We have introduced the random shock at the property level in our guiding example to represent individual landowner preferences or property characteristics. As such, correlation within grids and counties will also be introduced, since they contain multiple pixels within a property. Clustering standard errors relaxes the assumption of no correlation across observations within the spatial unit used for clustering [@jones2015]. 

Table \@ref(tab:did-cover) demonstrates how the presence of property level unobservables impacts coverage probability in the pixel-level DID model. The level at which standard errors are clustered clearly has an impact on coverage probability. When property level unobservables enter the DGP, clustering at the pixel level is ill-advised, as coverage probability declines rapidly.

<!--
TODO: Does it make more sense to combine tables 3 and 4?
-->
...
Table \@ref(tab:uoa-cover) demonstrates how coverage probabilities are impacted by property-level unobservables for TWFE models with aggregated units of analysis. The property level specification incurred the least bias with the introduction of property level disturbances (Figure \@ref(fig:property-uoa)). Table \@ref(tab:uoa-cover) now shows that while all three aggregated units of analysis provide coverage probability near the desired 95\% when $\sigma_p=0$, the property-level model is the only one that maintains coverage above 90% as $\sigma_p$ increases. 
...
kable(coverage_fe, format = "latex", row.names = FALSE,  booktabs = T,
      caption = "Coverage probabilities for pixel level, TWFE models using aggregated fixed effects in presence of property level unobservables", 
      col.names = NULL) %>% 
  add_header_above(c("unit of analysis"=1,"fixed effects"=1, "standard error structure"=1, "0"=1, "0.1" = 1, "0.2" = 1, "0.3" = 1), escape = TRUE, line =TRUE, bold = TRUE)%>%
  add_header_above(c(" "=1, " "=1, " "=1, "coverage probability ($\\\\sigma_p =$)"=4), escape = FALSE, line =FALSE, bold =FALSE)%>%
  kable_styling(font_size = 10, latex_options = c("HOLD_position", "striped"), 
...
Table \@ref(tab:fe-cover) demonstrates how coverage probabilities are impacted by property level unobservables for pixel level TWFE models with aggregated fixed effects.The pixel level model with property fixed effects incurred less bias than the pixel level models with other aggregated fixed effects. Table \@ref(tab:fe-cover) shows that the model with property fixed effects also outperforms the other models in this table in terms of coverage probability. 

Using \@ref(fig:summary) to compare across all models, we see that RMSE tends to increase across all specifications as $\sigma_p$ increases. The pixel-level model with property fixed effects tends to have the lowest RMSE whenever $\sigma_p$ is nonzero. In contrast, the property-level model and pixel-level DID tend to have the highest RMSE. When bias and coverage probability are comparable, it is preferable to use the model with lower RMSE. 

Overall, this section highlights the benefits of structuring one's econometric modeling decisions around the decision-making unit. In some instances, researchers do not have access to geospatial boundaries of the relevant decision-making unit. County and grid cell level aggregation lead to lower levels of bias and higher coverage probability than pixel level specifications when property unobservables are present. County level aggregation leads to the best coverage in the absence of property boundaries, meaning that a larger, alternative administrative unit may be the best alternative when an adequate number of observations are available. If constraints on geospatial boundary access prevent analysis at the scale of the relevant decision-making unit, a grid level model or one with grid fixed effects is likely to greatly improve bias and coverage probability relative to a pixel level DID. 

```{r summary, fig.cap="bias, distribution, and coverage of all models with standard errors clustered at the level of the model fixed effects. The pixel level DID is clustered at the pixel level. ", out.width = "100%", fig.pos='H', fig.show='hold'}
knitr::include_graphics("figs/summary_full1.png")
```

<!--
TODO: A few suggestions / questions for this figure
1. Probably best to just have the value for RMSE and deviation as numbers rather than bars? Would also make Fig 4 a bit easier to read - simultaneously see large difference between 1 and all others, and variation among the others.
2. Could DID just be expressed as treatment fixed effect?
3. Drop pixel FE since that's not being used in this figure?
4. Don't quite understand what a lack of dropped pixels means in the aggregated models. How are these pixels contributing to the deforestation rate calculation?
5. Should we add some models that use DID, but cluster SE at county or property? These seem to perform much better than the standard DID, so could be useful to show the best performing DID for context.
6. Illustrate changing sigma_p using lines to separate into sub-plots, move label to top-left of each sub-plot.
-->
