...
To do:
\begin{itemize}
\item figure captions/legends (did labels annd brackets, twfe colors, legends, and figure 2 alignment)
\item citations
\end{itemize}


## Abstract
Conservation practitioners need rigorous evidence measuring the effectiveness of proposed policy interventions. Increasingly, scientists are generating this evidence by applying methods of impact evaluation to remotely sensed data on land use change. Here we review this burgeoning literature to develop practical guidance for the design of econometric models quantifying conservation policy effectiveness. Using Monte Carlo simulations and analytical proofs, we demonstrate that many of the models employed for conservation impact evaluation are biased - the significance, magnitude and even direction of estimated effects from many studies may be incorrect. These errors threaten to undermine the evidence base that is increasingly used to inform conservation policy adoption and design. We demonstrate that careful design of econometric models can minimize this bias to better inform conservation.
...
<!-- Robert: Edit this paragraph -->

```{r, echo=FALSE,out.width="49%", fig.cap="Difference-in-Differences intuition in two time periods", fig.show='hold'}
knitr::include_graphics(c("figs/twoperiod_landscape.png","figs/trends.png"))
```
...
Several key decisions must be made by the researcher in the design of the econometric model for causal inference. We outline some of these design decisions including the choice between DID and TWFE models, the unit of analysis, functional form, and calculation of deforestation rates. A more in depth discussion of these model decisions can be found in the Appendix. As seen in Table 1, a wide variety of model parameterizations are used in the literature, and some of these may generate biased estimates. We focus on model decisions that are not consistent across the literature and for which little guidance currently exists. 

Researchers must decide on the unit of analysis for their econometric model, which comes with several implications both for the interpretation of results and consideration of spatial autocorrelation. Analyses using a binary pixel or plot are standard in the literature [@jones2015]. A pixel or grid cell level analysis may be preferable in contexts where there is no clear alternative unit of interest, however, the pixel is often times used without any explanation for why it is preferred to alternative options. This paper explores some of the potential pitfalls of using the pixel as opposed to an aggregated unit of analysis such as a grid cell or administrative unit.  

Other papers in the literature choose to conduct analysis at the level of the property or comparable decision making unit. This level of analysis matches the scale at which management decisions often occur [@carlson2018], and has been promoted in the context of PES programs that enroll at the property level. Using the entire area owned by an individual landowner or a larger adminitrative unit such as the county may provide the advantage of accounting for spillover effects that occur when the intervention spurs deforestation on other parts of the relevant unit [@blackman2018; @alix-garcia2014; @arriagada2012]. We consider how aggregation may impact researchers' analyses in the presence and absence of property level unobservables.  
...
<!-- Seems like this paragraph should come immediately after section on unit of analysis -->
...
### Binary Unit of Analysis
<!-- Broad comment: Maybe it makes sense to remove "binary unit of analysis" and "treatment of binary outcome" sections? I think we could collapse these points into a few sentences at the start of the "issues with TWFE" and a reference to a plot in the appendix. Unless we can find examples of papers that don't drop observations, this seems like a bit of a strawman, and we'd want to focus limited words on issues that people are actually somewhat divided about  -->
<!-- I'd integrate this paragraph into either the previous or the next sub-section. Reads a bit strange with this sitting on its own since there's not a clear take-away  -->
...
<!-- make overlapping distributions the same color to make more clear they are overlapping
-->
...
<!-- Careful with % versus percentage point in this paragraph -->
...
While pixel-level analyses are not feasible in TWFE regression models, the grid-level is a viable substitute for comparable interpretation. The grid cell may also offer a solution when there is no clear alternative unit of interest such as a property or broader administrative unit. In cases where the researcher has two groups and two distinct time periods, a pixel level analysis is still feasible using the simple DID model.

### Functional Form
Given that all of the outcome metrics used in the studies presented in Table 1 are generated from maps of binary, pixel-level deforestation events, several researchers have suggested that non-linear functional forms may be preferred over linear models. For example, models estimated using individual pixels may be better represented by a probit or logit functional form (XX - Add refs), whereas models that count deforested pixels within a broader unit of observation may be best modeled using a Poisson distribution [@burgess2012; @busch2015a; @carlson2018]. However, these non-linear functional forms can complicate interpretation of regression coefficients [@gomila2019]. We explore whether the use of non-linear functional forms reduce bias in ATT estimates. We find that, in many cases, a simple Linear Probability Model (LPM) results in less bias than alternate, non-linear functional forms. For example, Figure 6 shows the distribution of model bias resulting different model structures in our guiding parameterization. 
<!-- I wonder whether we should introduce functional form as the final example, so we can also compare the poisson against a simple linear model for aggregated variables? -->
<!-- Maybe move this to the caption of the figure?:
We use DID regression models at the pixel level in this section to avoid the aforementioned issues with pixel level TWFE models. -->

```{r, echo=FALSE,out.width="75%", fig.cap="Distribution of estimates produced by different functional forms", fig.pos='H', fig.align='center'}
knitr::include_graphics("figs/func1.png")
```
<!-- I think the "DID" distribution should be labeled "Linear probability model" or LPM, since all of the four examples are DID models, correct? 
Also, only cases I can think of when Poisson was used was after aggregating across lots of pixels. Maybe break out into separate plots - one comparing LPM to logit / probit; and one comparing linear / log-linear grid cell model to poisson grid cell model?
-->

We also explore how functional form choice may depend on the deforestation rates in a given setting, since functional form choice may have more of an impact in low event probability scenarios. Figure 7 explores how bias behaves as a function of the magnitude of our initial parameters. Here, we are scaling the set of initial parameters, while keeping their relative magnitudes the same. We see that the linear DID models still outperforms the non-linear functional forms with extremely low deforestation rates. When deforestation rates become high, we see that the non-linear models begin to outperform the linear DID model, however it is unlikely to encounter a setting with deforestation rates this high. For reference, the linear DID still outperforms the nonlinear models when the parameter set is three times as large as the guiding example, where the baseline deforestation rate in the intervention area is 15/%. 

```{r, echo=FALSE,out.width="75%", fig.cap="Bias depends on the magnitude of initial landscape parameters", fig.pos='H', fig.align='center'}
knitr::include_graphics("figs/vary_func.png")
```
<!-- Think this needs a bit more clarity. "Ratio of parameters" is a bit too vague. Also, does it make sense to start at 1 and increase from there? Current plot draws a lot of attention to the volatility as you approach 0 - is that actually an important consideration, or do we want people to focus on the fact that there's an increasing trend-line for many of the variables?
-->

The underlying DGP and error structure play a role in determining which functional form leads to the least bias in a given scenario. Alternative parameterizations may affect which functional form is optimal in a given circumstance. Further, one can imagine a slightly different DGP in which the preferred functional form is nonlinear. Therefore, authors, regardless of the preferred specification, should justify their choice, taking into account the DGP that underlies their specific context. We also recommend that authors ensure the result is robust to alternate functional forms. 

### Calculating Deforestation Rates

Upon aggregating data, the researcher must determine how to calculate deforestation rates in the outcome. With no clear guidance on how this deforestation rate should be computed, there have been a variety of techniques used. We examine three common methods. 
...
Lastly, we consider a formula derived from the Compound Interest Law that has also been used in recent studies [e.g. @ruggiero2019]. Outcome 3) is given by:
...
### Unit of analysis
<!-- Might need to better motivate what the benefits of the TWFE model actually are -->
...
We find little evidence that any one level of aggregation is consistently preferrable in terms of bias, meaning that RMSE and coverage probability play a larger role as an evaluation metric. We explore the bias, RMSE, and coverage probabilities corresponding to different levels of aggregation in the context of our guiding example. Figure 9 shows that the bias of the estimates is not critically different depending on level of aggregation, however, the distributions are varied. RMSE is highest with property level aggregation due to the heterogeneity in property sizes. 

Coverage probabilities are impacted by the distribution of the estimates, bias, and standard error structure. We compare coverage at each level of aggregation in Table 2. The pixel level analysis leads to a poor coverage probability, while the aggregated analyses lead to relatively greater coverage. We have not yet introduced any structure into the DGP that would warrant clustering. This is demonstrated by the coverage probabilities when $\sigma_p = 0$. Despite the clear differences in the distributions of estimates, both pixel and property-level analyses yield the desired 95\% coverage probabilities when $\sigma_p = 0$. When the desired coverage probability is achieved, it is preferable to use models with a lower RMSE. 


<!-- Might be interesting to include pixel clustered at county or grid. Basically, when we don't have any information on property boundaries, how well does a pixel-level model perform if we cluster at county? -->
...
Property level unobservables may impact both treatment effect estimates and coverage probabilities. This is likely to be a factor when land use decisions are made at the property level or the intervention seeks to alter underlying landowner incentives. We introduce an additional error term to the initial DGP that varies at the property level in order to account for these unobservables. We explore the cases when the property level disturbances are generated with $\sigma_p = .05$, $0.15$, and $0.25$. Table 3 shows that property level unobservables negatively impact coverage at every level with classical standard errors. The property level analyses suffer the least from this reduced coverage probability, implying that aggregating to the level of the relevant decision making unit is often preferrable.

<!-- I'm still left wondering whether the RMSE difference for the property analysis that we raise in the previous section becomes less of an issue once you have property-level disturbances. -->
...
Examining the below table shows that clustering to the respective level of aggregation does indeed improve the coverage probability in the presence of property level unobservables. Property level aggregation leads to relatively preferred coverage probabilities, regardless of the standard error structure. When using the pixel as the unit of analysis, clustering standard errors at the proper level leads to the desired coverage probability as well. These results highlight the benefit of using the relevant decision-making unit as the unit of analysis as well as clustering standard errors at that level. 
...
## Discussion
<!-- Incorporate a sentence to reflect the TWFE result, something along the lines of: Despite past guidance to the contrary, models that incorporate fixed effects for individual points are unable to yield unbiased estimates of a policy's ATT when applied to irreversible, binary observations of deforestation. -->
...
\end{align}
, where $F_{it}$ and $F_{i,t-1}$ are the forest cover at times $t$ and $t-1$, respectively. This calculation is used consistently in the literature [e.g. @carlson2018; @busch2015a] and has been referred to as the rate of deforestation and annual deforestation percentage. 

Some authors have also calculated the deforestation rate in relation to the initial observed level of forest cover, presumably replacing $F_{it}$ with $F_{i0}$, the baseline forest cover, in equation (1)
\begin{align}
...
In addition to the guiding example, we explore the performance of the deforestation rate formulas in an example with varying observation period lengths. This demonstrates the performance of the formulas when the landscape's forest cover changes significantly relative to when it is first observed. A longer study period means that over the course of the study period, relatively more deforestation will occur relative to a shorter period, assuming the same landscape parameters. We set $trend =0$ and $ATT=0$ to show a basic case where the intervention had no effect and deforestation rates in the treated and untreated group are stable though time.
...
### Issue using formula 2) to calculate deforestation rate
