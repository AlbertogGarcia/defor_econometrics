...
Other literatures have accounted for irreversible outcomes of interest such as mortality (e.g., @friedman2013), ... (e.g., ), or technology adoption (e.g., @beyene2013). Many researchers do so by using survival models, where the researcher is interested in the time each unit remains in the sample. In our context, this corresponds to the time each pixel survives until deforestation. 

We explore whether survival models are also a viable approach in the deforestation setting. Figure \@ref(fig:cox-fig1) shows difference-in-differences estimates from a cox proportional hazards model as well as the pixel OLS DID (regression 1) with a pre-period and post-period of 10 years. The hazard rate ratio of interest, 

$$ HRR = \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=1]}$$

has been converted into the $ATT$ for the Cox proportional hazards model (Model and conversion process in Appendix \@ref(coxph)). 

```{r cox-fig1, fig.cap=" bias, distribution, and coverage of Cox proportional hazard DID relative to pixel DID using OLS.",  out.width="100%"}
knitr::include_graphics("figs/coxph1.png")
```
We see that the Cox proportional hazards model is a viable approach to recovering the impact of the conservation intervention in this context. However, while survival analysis recovers the appropriate parameter, there are several reasons why survival models may not be preferable in the context of estimating the impacts of conservation interventions on deforestation rates.
...
Figure \@ref(fig:grid-fe) shows how bias, coverage probability, and RMSE depend on grid size in pixel-level TWFE regressions with grid unit fixed effects (Regression 3, where $j$ represents grid cell $j$). Particularly undersized or large grid cells tend to perform relatively worse in terms of bias, coverage and RMSE. We find that too high of resolution (too small of grid cells) analyses suffer from potentially severe bias, and low resolution (too large of grid cells) analyses may be both biased and suffer from imprecise estimation and inference.
...
```{r grid-uoa, message = FALSE, results = FALSE, warning = FALSE, fig.align="center", fig.cap="Regression 3 with grid unit fixed effects; bias, coverage, and rmse depend on grid size when property level unobservables are present. Note: x-axis begins at 2x2 grid cells to ensure grid cells are not simply pixels.", out.width = "85%", fig.width = 12, fig.height = 6, fig.pos='H', fig.show='hold'}
...
Our hazard rate ratio of interest is 

$$ HRR_{ATT} = \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=1]}$$

Here, the numerator represents the deforestation rate within treated areas after the intervention, while the denominator represents the counterfactual deforestation rate in those areas after the intervention, had the intervention never ocurred. Thus the $HRR$ of interest represents the ration of two deforestation rates. 

The $ATT$ is expressed 

$$ ATT = E[y_{ivt}(1)  |  t\geq t_0, D_i=1] - E[y_{ivt}(0)  |  t\geq t_0, D_i=1]$$.

Note that we observe the $E[y_{ivt}(1)  |  t\geq t_0, D_i=1]$ term, which we denote $d_{obs}$.
Denote the unobserved counterfactual term, E[y_{ivt}(0)  |  t\geq t_0, D_i=1], $d_{cf}$.

We see that $d_{cf} = \frac{d_{obs} }{HRR}$, and thus that 

$$ATT = d_{obs} - \frac{d_{obs} }{HRR}$$

In order to estimate the $HRR$ using the Cox proportional hazards model, we must combine coefficients from separate Cox regressions. 



Our three needed hazard rate ratios are:
\begin{align*}
...
The desired $HRR_{ATT}$ can then be found by:

$$HRR_{ATT} = \frac{1}{1/HRR_1 + 1/HRR_2 - (1/(HRR_2*HRR_3))}$$

Each of $HRR_1$, $HRR_2$, and $HRR_3$ can be estimated individually by Cox regressions. Specifically,
...
$$h(t) =\lambda_0(t) exp(\delta 1\{ t \geq t_0\} )$$, subsetting to only the untreated group.
\end{align*}
, where $h(t)$ is the hazard rate of deforestation at time t. Then
\begin{align*}
E[exp(\hat{ \alpha })] &= HRR_1\\
E[exp(\hat{ \beta })] &= HRR_2\\
E[exp(\hat{ \delta })] &= HRR_3
\end{align*}
