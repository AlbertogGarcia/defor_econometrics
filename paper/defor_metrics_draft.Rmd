...
Here, we introduce a setting in which groups of units receive treatment at different times (full DGP can be found in Appendix ...). We consider three groups: an early group, a late group, and a never-treated group. Each group experiences differing pre-treatment deforestation rates (7\%, 4\%, and 2\% for the early, late, and never-treated groups, respectively) and no time trend. The $ATT$ for each treated group is $-0.02$ representing a decrease in the deforestation rate of treated units by 2 percentage points. This effect does not increase or diminish over time, and common trends is satisfied by construction. Figure \@ref(fig:observed-multiGT) shows the observed deforestation rates ($E[y^o_{ivt}]$) from one iteration of our simulation in this setting. 
