...
\begin{enumerate}
\item In the case of an pixelated (binary) forest cover outcome, observations must be dropped in the periods after a pixel first becomes deforested. \textbf{supported by analytical proof and Monte Carlo simulations}. 
\item Pixel level TWFE regression models with a binary forest cover outcome do not identify the $ATT$, but in fact, identify a simple post-treatment difference in outcomes ($ATT$ plus the pre-treatment difference in outcomes). \textbf{supported by analytical proof and Monte Carlo simulations}.
\item Property-level unobservables negatively impact model bias, coverage, and RMSE when the unit of analysis does not match the true scale of the deforestation process. \textbf{supported by Monte Carlo simulations}.
\item When unobservables operate at several different scales, models that match the scale of the primary driver of deforestation heterogeneity outperform the alternatives. \textbf{supported by Monte Carlo simulations}.
\item Choice of model functional form may play a larger role in particularly high and low deforestation settings. \textbf{supported by Monte Carlo simulations}.
\end{enumerate}
...
## Using a binary forest cover outcome
...
kable(coverage_perr[,2:7], format = "latex", row.names = FALSE, 
  ...
  add_header_above(c(" "=1, " "=1, "$\\sigma_p = 0$"=1, "$0.05$" = 1, "$0.15$" = 1, "$0.25$" = 1), escape = FALSE, line =FALSE, bold =FALSE)%>%
  kable_styling(font_size = 10, latex_options = c("HOLD_position"),bootstrap_options = c("striped", "hover", "condensed", "responsive"),#full_width = TRUE
                position = "center")
...
In any given landscape, several different processes may impact land-use or land use change. While landowner preference may play a significant role in landscape heterogeneity, unobservable county or ecosystem characteristics may drive observed outcomes as well. It has been advised to aggregate to the unit that drives this process of land use change [@avelino2016], however, little guidance exists for situations in which multiple drivers or landscape processes may be at work simultaneously. 

In order to determine the tradeoffs associated with different units of analysis in the presence of spatially distinct but concurrent landscape processes, we now introduce unobservable perturbations at the county level in addition to our property level unobservables. Figure 4 shows how coverage probability may be impacted by the introduction of an additional source of unabservable heterogeneity. As the size of the standard error of the county level perturbations, $\sigma_c$, increases, the property level model sees decreased coverage relative to the desired 95% coverage probability, whereas the county level model does not see as drastic of a decline in performance. It may therefore be optimal to aggregate to the level that is the best determinant of the observed heterogeneity. 

```{r, fig.cap="Performance of property versus county level aggregation in the presence of unbservables at multiple scales. The standard error of the property level unobservables, $\\sigma_p$ is equal to $0.15$. Standard errors are clustered at the level of aggregation.", fig.pos='H', fig.show='hold'}
knitr::include_graphics("figs/coverages_county.png")
```

<!-- ```{r table4, results = "asis", fig.show='hold'} -->
<!-- coverage_cp <- read.csv("coverages_county.csv")[,2:8] -->

<!-- kable(coverage_cp, format = "latex", row.names = FALSE,  -->
<!--       caption = "Table of coverage in presence of unobservables at multiple scales. The standard error of the property level unobservables, $\\sigma_p$ is equal to $0.15$. Standard errors are clustered at the level of aggregation.",  -->
<!--       col.names = NULL) %>%  -->
<!--   add_header_above(c("unit of analysis"=1, "coverage probability" = 6), escape = TRUE, line =TRUE, bold = TRUE)%>% -->
<!--   add_header_above(c(" "=1, "$\\sigma_p = 0$"=1, "$0.05$" = 1, "$0.1$" = 1, "$0.15$" = 1, "$0.2$" = 1,"$0.25$" = 1), escape = FALSE, line =FALSE, bold =FALSE)%>% -->
<!--   kable_styling(font_size = 10, latex_options = c("HOLD_position"),bootstrap_options = c("striped", "hover", "condensed", "responsive"),#full_width = TRUE -->
<!--                 position = "center") -->
...
## Functional Form
Given that all of the outcome metrics used in the studies presented in Table 1 are generated from maps of binary, pixel-level deforestation events, several researchers have suggested that non-linear functional forms may be preferred over linear models. For example, models estimated using individual pixels may be better represented by a probit or logit functional form [@blackman2015], whereas models that count deforested pixels within a broader unit of observation (i.e. grid cell) may be best modeled using a Poisson distribution [@burgess2012; @busch2015a; @carlson2018]. However, these non-linear functional forms can complicate interpretation of regression coefficients [@gomila2019]. We evaluate whether the use of non-linear functional forms reduce bias in ATT estimates in our setting. We find that, with our DGP, a simple Linear Probability Model (LPM) often results in less bias than alternate, non-linear functional forms. For example, Figure 7 shows the distribution of model bias resulting from different model structures in our guiding parameterization. 
