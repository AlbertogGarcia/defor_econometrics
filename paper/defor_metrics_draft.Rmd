...
$$\widehat{HRTT} = \frac{1}{1/exp(\hat{ \alpha }) + 1/exp(\hat{ \beta }) - 1/(exp(\hat{ \beta })*exp(\hat{ \delta }))}$$
...
df_summary <- specchart_long %>%
  filter(weights == 0 & (treatment.fe == 0 | se_pixel == 1)) %>%
  mutate_at(vars(bias, cover), as.numeric
         )%>%
  mutate_at(vars(pixel, grid, property, county, pixel.fe, grid.fe, property.fe, county.fe, treatment.fe, cox, X.HRRT.estimator, se_pixel, se_grid, se_property, se_county), ~ as.logical(as.integer(.))
  )%>%
  filter(is.na(notes))%>%
  group_by(pixel, grid, property, county, pixel.fe, grid.fe, property.fe, county.fe, treatment.fe, cox, X.HRRT.estimator, se_pixel, se_grid, se_property, se_county)%>%
...
df_summary <- rbind(df_summary[10,], df_summary[5,], df_summary[7:9,], df_summary[1:3,], df_summary[6,], df_summary[4,])
...
We explore whether survival models, including our proposed estimator, are a viable approach to recover treatment effects in the deforestation setting. Because the numerator of $HRTT$, $E[y_{ivt}(1)  |  t\geq t_0, D_i=1]$, is observable, we can recover the $ATT$ using the observable term and $HRTT$:

$$ATT = E[y_{ivt}(1)  |  t\geq t_0, D_i=1] - \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1] }{HRTT}$$


We see in column 9 that the Cox DID regression (Regression 3) does not recover the impact of the conservation intervention in this context. However, $\widehat{HE}$, as seen in column 10, successfuly recovers our parameter of interest.
...
df_summary <- specchart_long %>%
  filter(is.na(notes) & pixel.fe == 0 & weights == 0 & (treatment.fe == 0 | se_pixel == 1) & (cox == 0 | X.HRRT.estimator ==1)) %>%
  mutate_at(vars(bias, cover), as.numeric
         )%>%
  mutate_at(vars(pixel, grid, property, county, pixel.fe, grid.fe, property.fe, county.fe, treatment.fe, X.HRRT.estimator, se_pixel, se_grid, se_property, se_county), ~ as.logical(as.integer(.))
  )%>%
  group_by(pixel, grid, property, county, pixel.fe, grid.fe, property.fe, county.fe, treatment.fe, X.HRRT.estimator, se_pixel, se_grid, se_property, se_county)%>%
  summarise(RMSE = rmse(bias, 0),
            q05 = quantile(bias, probs = .05),
            q95 = quantile(bias, probs = .95),
            Bias = mean(bias),
...
schart(as.data.frame(schart_results), labels = labels, ylim = ylim, axes = FALSE, index.ci=index.ci, ylab="", 
       leftmargin = 16, col.est = c(palette$dark, palette$red), heights = c(3.5,2.25), cex = c(1.75,1.75))
...
full_summary <- readRDS("results/summary_full.rds") %>%
  mutate_at(vars(bias, cover), as.numeric)%>%
  filter(std_p==0.5 & is.na(notes) & pixel.fe == 0 & weights == 0 & cox == 0 & (treatment.fe == 0 | se_pixel == 1))%>%
  group_by(pixel, grid, property, county, grid.fe, property.fe, county.fe, treatment.fe, se_pixel, se_grid, se_property, se_county)%>%
  summarise(Bias = mean(bias),
            cover = mean(cover),
            RMSE = rmse(bias, 0),
...
schart_results <- select_results %>% 
  select(c(Bias, q05, q95, pixel, grid, county, property, treatment.fe, grid.fe, county.fe, property.fe))%>%
  mutate_at(vars(pixel, grid, county, property, treatment.fe, grid.fe, county.fe, property.fe), ~as.logical(as.numeric(.)))

index.ci <- match(c("q05","q95"), names(schart_results))

labels <- list("Unit of analysis:" = c("pixel", "grid","county", "property"),
               "Fixed effects:" = c("treatment FE", "grid FE", "county FE", "property FE")
               )
...
```{r pweight, fig.align="center", results = FALSE, warning = FALSE, message = FALSE, fig.cap="weighting recovers landscape scale interpretation", fig.pos='H', fig.width = 9, fig.height = 5, out.width= "100%", fig.show='hold'}
...
full_summary <- summary_full %>%
  ...
  filter(is.na(notes) & pixel.fe == FALSE & (treatment.fe == 0 | se_pixel == TRUE))%>%
