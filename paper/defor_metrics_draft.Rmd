...
Other literatures have accounted for irreversible outcomes of interest such as mortality (e.g., @friedman2013), ... (e.g., ), or technology adoption (e.g., @beyene2013). Many researchers have opted to use survival models, where the researcher is interested in the time each unit remains in the sample. In our case, this corresponds to the time each pixel survives until deforestation. 
...
has been converted into the $ATT$ (Model and conversion process in Appendix \@ref(coxph)). 
...
```
We see that the Cox proportional hazards model is also a viable approach to recovering the impact of the conservation intervention in this context. However, while survival analysis recovered the appropriate parameter, there are several reasons why survival models may not be preferable in the context of estimating the impacts of conservation interventions on deforestation rates. 

<!-- One of the primary reasons for the use of survival analysis is censoring. This occurs when the researcher has partial information about the subjects’ survival times but does not have access to precise event times. -->

As we will show in the next sections, when the process of deforestation operates at a scale different than the pixel, an alternatiive unit of analysis may be preferable. We explore how some of these factors influence the relative performance of various models in the following sections. 
