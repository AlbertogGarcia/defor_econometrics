...
Several key decisions must be made by the researcher in the design of the econometric model for causal inference. We outline some of these design decisions including the choice between DID and TWFE models, the unit of analysis, and functional form. A more in depth discussion of these model decisions can be found in the Appendix. As seen in Table 1, a wide variety of model parameterizations are used in the literature, and some of these may be biased estimators of the treatment effect. We focus on model decisions that are not consistent across the literature and for which little guidance currently exists. 
...
```{r, fig.cap="Distribution of estimates produced by level of aggregation", fig.pos='H'}
...
<!-- Although we do not explore grid cell or property size/weighting at this point, it might be good to give some frame of reference. I'm thinking we pretend each pixel is 30x30 and then determine how many pixels each property/grid/etc. are made of -->
