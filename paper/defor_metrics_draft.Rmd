...
```{r, echo=FALSE,out.width="49%", fig.cap="deforestation observed in the first vs. second period in treated and untreated units", fig.show='hold', fig.align='center'}
knitr::include_graphics(c("landscape_figs/landscape_did.png", "landscape_figs/barchart.png"))
```
<!-- FIGURE NOTES: 
I will adjust the boxplot colors, but does this plot get at the did well enough?

Robert response: I think explicitly showing differences in each time period, and then the differences between these is useful. Although neither of these is very pretty (usually part of a slide with fewer elements visible at any time) here are examples of two-period (https://www.dropbox.com/s/8zqm7n9q4itubmp/conceptual_example.PNG?dl=0) and trend comparisons (https://www.dropbox.com/s/j9cdunjqc9rqri1/conceptual_example2.PNG?dl=0) 
-->
...
-->
<!-- 
Robert comment: I think we need a better framing to put the following text into context - how did we settle on these parameters as the focus of our attention? 1-3 paragraphs that highlight that these are critical choices that the researcher needs to make. In addition, if we do target Conservation Letters, maybe we move the more technical and detailed discussion of these elements to an appendix?
-->
#### Unit of analysis
...
<!-- Albert:
unclear whether functional form might have to do with the bias inherent to the did. This might be something worth discussing below, but I'm not sure of any intuition behind why it would or wouldn't -->
...
We employ a series of Monte Carlo simulations to (1) generate synthetic landscapes with known policy effectiveness and (2) analyze the performance of different econometric models in estimating the policy's known impact. Our landscape consists of administrative units that are either untreated $(D=0)$ or are assigned to a conservation treatment $(D=1)$. We observe deforestation in two, even-length periods, a pre-treatment $(t < t_{0})$ and a post-treatment $(t \ge t_{0})$ period. The data generating process underlying our Monte Carlo simulations begins with the assignment of four parameters: The mean, pre-treatment period deforestation rate for untreated units, $baseline_0$; the mean, pre-treatment period deforestation rate for treated units, $baseline_1$; untreated units' mean change in deforestation rates occurring between the pre- and post-treatment periods, $trend$; and lastly, the average treatment effect of the policy on the treated units, $ATT$. The researcher's primary goal is to estimate ATT. 
<!-- Robert: Should "trend" apply to both treatment and control units? I.e., are we assuming parallel trends in the absence of treatment, or do we need to add a further parameter to capture this source of confounding? Current model structure leaves this ambiguous... -->

Rather than directly observing each unit's <!-- "unit" is a bit ambiguous here. What should the reader be imagining at this point? Properties, counties, parcels, an individual tree? --> deforestation rate in each time period, the researcher observes annualized maps depicting pixel-level, binary deforestation $(y_{it}\ \epsilon \ 0, 1)$. We follow [@kerr; @pfaff2004] and model these binary deforestation events as a function of each pixel's unobservable value along a continuous, latent variable $(y^*_{it})$ indicating the utility of deforesting pixel $i$, in property $v$, in year $t$. We define this latent variable as:
<!-- Robert: I tried to revise to clarify that the latent deforestation suitability influences actual deforestation, and isn't an attempt to determine whether the binary event is detected. I think there are several places where the paper conflates the latent variable (suitability for deforestation) with detectability (amount of deforestation occurring in a pixel before binary metric flips to one). For example, I've highlighted another area where this is an issue in the first paragraph of the results. -->

\begin{align}
y^*_{ivt} = \beta_0 + \beta_1 \mathbbm{1}\{  D_i = 1  \} +\beta_2 \mathbbm{1}\{  t \geq t_0  \} +\beta_3 \mathbbm{1}\{  D_i = 1  \} \mathbbm{1}\{  t \geq t_0  \} + \gamma_v + \alpha_i +u_{it}
\end{align}

The four $\beta$ coefficients dictate how the utility of deforestation evolves over the two time periods $(\mathbbm{1}\{  t \geq t_0  \})$, across the control and treated units $(\mathbbm{1}\{  D_i = 1  \})$. These coefficients can be defined as a transformation of the four parameters assigned by the researcher ($baseline_0$, $baseline_1$, $trend$, $ATT$, derivation provided in Appendix 1). In addition, we assume that the value of deforestation is influenced by time-invariant random disturbances at the scale of individual pixels ($\alpha_i \sim N(0, \sigma_a^2)$) or properties ($\gamma_v \sim N(0, \sigma_v^2)$), as well as time-varying, pixel-scale disturbances ($u_{it} \sim N(0, \sigma_u^2)$). These disturbances can represent a variety of spatial and temporal processes including, for example, the biophysical characteristics of a location, or the preferences of a property owner. 


We assume that pixels with a positive utility of deforestation are cleared, resulting in the following mapping from the latent $y^*_{it}$ to the observed $y_{it}$ variable:
\begin{align}
 y_{it} = \begin{cases} 
      1 & y^*_{it} > 0  \\
      0 & otherwise
...
```{r parameterization table, echo = FALSE, results = "asis", fig.show='hold'}

param_table <- read.csv("param_table.csv")[,2:3]

kable(param_table[1:4,], format = "latex", row.names = FALSE,
      caption = "guiding example parameterization"
      , booktabs = TRUE
      #col.names = c("unit of analysis")
) %>% 
  add_header_above(c("$\\\\sigma_a=0.1$, $\\\\sigma_v=0.25$" = 2 ), escape = FALSE, line = FALSE) %>%
  row_spec(0,bold=TRUE) %>% 
...
<!-- Albert: I this this section addresses the issue well, but we could emphasize the importance of this finding or the prevalence in the literature to make more exciting if we feel it is needed -->

Despite widespread use of pixel level analyses, they are problematic in the context of TWFE regression models. In fact, the TWFE model yields the post-treatment difference in outcomes (single difference), rather than the desired ATT. We provide two forms of evidence to support this claim: (1) evidence from our Monte Carlo simulations and (2) an analytical proof found in the appendix. The result stems from the fact that the TWFE regression is only able to identify off of pixels that are not dropped over the course of the observation period. <!-- Robert: I don't quite understand the preceding sentence. Problem is really just the pixels that are dropped prior to treatment, right? --> Thus the pre-treatment period deforestation rates are not accounted for in the TWFE estimates. 
...
<!-- ALbert: We could also get rid of the estimates keeping the observations if that would make things more clear. I do like that it shows they are generally equivalent though 
Robert: Maybe add textures to the plot to make it easier to see the overlap between the last two groups?
-->
...
```
<!-- Robert: Table might need more clarification -->
...
As seen in Figure 7, outcome 1 results in the least bias in our guiding example. The other outcomes result in relatively greater bias, although this difference seems minimal. We express concern surrounding the use of the initial observed baseline forest cover in the calculation of deforestation rates, as in formula (2). In settings with high deforestation rates, this outcome may overestimate the deforestation rate. As the rate is calculated in periods far away from the baseline, the deforestation rate will be increasingly high. This may impact the viability of the analysis, as the rate becomes artificially inflated in later years. As seen in Appendix T2, outcome two is the least robust to alternate landscape parameterizations. 

<!-- Albert: Regarding formula 2, maybe we could show the rate inflating as deforestation becomes higher somehow, to back up the claim that it may be inflted 
Robert: Yes, I like that idea. Maybe three panels in which you show the same figure, but for different deforestation rates? -->
...
<!-- Albert: Should RMSE be discussed here to say that the narrower distributions are preferable when coverage is the same? 
Robert: I think that's a good idea, but then we'd probably want to describe it in the methods discussion of evaluation?
-->
...
<!-- Albert: I've moved the functional form section below the others in order to reintroduce consideration of the "classification" bias. I did this because in Alix-Garcia's presentation, certain functional forms performed better. There is no consistently better functional form here, but I wanted to try to address that potential at least briefly. The con is that it is perhaps out of place and I wonder if it highlights the question of why we netted it out in the other sections -->

We explore the effect of using non-linear functional forms. We use DID regression models at the pixel level in this section to avoid the aforementioned issues with pixel level TWFE models . We also reintroduce the consideration of bias due to classification error in the dgp in order to explore whether alternative functional forms may help to mitigate this bias. Coefficient estimates are converted into the $ATT$ using the \textbf{mfx} package in R, which takes into account the binary nature of the treatment variable. 
...
The underlying DGP and error structure play a role in determining which functional form leads to the least bias in a given scenario. We find that alternative parameterizations do affect which functional form is optimal in a given circumstance, as seen in Table XX. Therefore, authors, regardless of the preferred specification, should justify their choice. As is common in the literature already, we advise that authors ensure the result is robust to different choices of functional form. We leave a thorough discussion of the choice of functional form in the presence of classification error to future work, but highlight the importance of this area for exploration.  

```{r outcome table, echo = FALSE, results = "asis", fig.show='hold'}

func_table <- read.csv("func_results.csv")[,2:4] %>%
  cbind(read.csv("func_results2.csv")[,3:4])%>%
  cbind(read.csv("func_results3.csv")[,3:4])
  

kable(func_table, format = "latex", row.names = FALSE,
      caption = "functional form across landscape parameterizations"
      #, booktabs = TRUE
      #col.names = c("unit of analysis")
) %>% 
  add_header_above(c("", "$trend = -0.005$" = 2, "$trend = 0.005$" = 2, "$trend = 0.01$" = 2), escape = FALSE, line =FALSE) %>%
  add_header_above(c("", "$baseline_1 = 0.04$" = 2, "$baseline_1 = 0.02$" = 2, "$baseline_1 = 0.05$" = 2), escape = FALSE, line = FALSE) %>%
  add_header_above(c("", "$baseline_0 = 0.02$" = 2, "$baseline_0 = 0.05$" = 2, "$baseline_0 = 0.02$" = 2), escape = FALSE, line =FALSE) %>%
  add_header_above(c("", "$ATT = -0.01$" = 2, "$ATT = 0.01$" = 2, "$ATT = -0.03$" = 2), escape = FALSE, line = FALSE) %>%
  add_header_above(c("", "500 datasets of 10,000 pixels, $\\\\sigma_a=0.1$, $\\\\sigma_v=0.25$" = 6), escape = FALSE, line = FALSE) %>%
  row_spec(0,bold=TRUE) %>% 
  kable_styling(font_size = 10, latex_options = c("HOLD_position"),
                #full_width = TRUE
                position = "center"
...
### deforestation rate calculation robustness to alternate parameterizations

```{r func table, echo = FALSE, results = "asis", fig.show='hold'}

outcome_table <- read.csv("outcome_results.csv")[,2:4] %>%
  cbind(read.csv("outcome_results2.csv")[,3:4])%>%
  cbind(read.csv("outcome_results3.csv")[,3:4])
  

kable(outcome_table, format = "latex", row.names = FALSE,
      caption = "outcome calculation across landscape parameterizations"
      #, booktabs = TRUE
      #col.names = c("unit of analysis")
) %>% 
  add_header_above(c("", "$trend = -0.005$" = 2, "$trend = 0.005$" = 2, "$trend = 0.01$" = 2), escape = FALSE, line =FALSE) %>%
