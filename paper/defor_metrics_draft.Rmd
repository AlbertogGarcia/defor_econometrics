...
<!-- JUST ADDED TEXT TO GIVE INTUITION BEHIND PROOF AND JUST ADDED BACK APPENDIX REFERENCE TEXT SINCE IT IS NOW IN APPENDIX -->
...
Other literatures have accounted for the irreversibility in the outcome of interest by using survival models. The irreversible outcomes of interest may be mortality (e.g., @friedman2013), ... (e.g., ), or technology adoption (e.g., @beyene2013). In these cases, the researcher is interested in the time each unit remains in the sample, which in our case, corresponds to the time each pixel survives until deforestation. Figure \@ref(fig:cox-fig1) shows difference-in-differences estimates from a cox proportional hazards model as well as the pixel OLS DID (regression 1), where the hazard rate ratio of interest, 
...
twfe_comp <- readRDS("TWFE_comp.rds")%>%
...
```{r alt-param, fig.align="center", fig.cap="Property-level specification still outperform others in presence of property unobservables with alternative landscape parameterizations", out.width = "70%", fig.pos='H', fig.show='hold'}
...
\tau_{1,a} = F^{-1}(ATT_{1,a} + F(\beta_{0,a} + \beta_{1,a} + \beta_{2,a} +\tau_{0,a}) )- \beta_{0,a} - \beta_{1,a}  - \beta_{2,a} -\tau_{0,a}\\
\tau_{2,a} = F^{-1}(ATT_{2,a} + F(\beta_{0,a} + \beta_{1,a} + \beta_{2,a} +\tau_{0,a}+\tau_{1,a}) )- \beta_{0,a} - \beta_{1,a}  - \beta_{2,a} -\tau_{0,a}-\tau_{1,a} \\
\end{align*}
