...
In many instances, researchers do not have access to geospatial boundaries of these relevant administrative units. In these cases, utilizing a grid cell or alternative administrative unit to capture spatial heterogeneity is likely preferable to pixel level DID models for purposes of both estimation and inference. The scale of this grid cell or alternative unit relative to the process of land use change is important. Grid cell size plays a role in bias, RMSE, and coverage probability, particularly when property level unobservables are present. 

Figure \@ref(fig:grid-fe) shows how bias and coverage probability depend on grid size in pixel-level TWFE regressions with grid unit fixed effects (Regression 3, where $j$ represents grid cell $j$). The top panel of Figure \@ref(fig:grid-fe) shows how bias is affected by grid size. Note that even in the absence of property-level unobservables ($\sigma_p = 0$), too small of grids result in biased estimates of the $ATT$. The introduction of these unobservables exacerbates the issue, causing undersized grids to suffer even more. The bottom panel of Figure \@ref(fig:grid-fe) shows how coverage probability is affected by grid size.

```{r grid-fe, fig.align="center", fig.cap="bias depends on grid size when property level unobservables are present.", out.width = "65%", fig.pos='H', fig.show='hold'}
knitr::include_graphics(c("figs/bias_gridsize_fe.png", "figs/cover_gridsize_fe.png"))
```

Figure \@ref(fig:grid-uoa) shows how bias and coverage probability depend on grid size in grid-level TWFE regressions (Regression 4, where $j$ represents grid cell $j$). The top panel of Figure \@ref(fig:grid-uoa) shows how bias is affected by grid size. Note that even in the absence of property-level unobservables ($\sigma_p = 0$), too small of grids result in biased estimates of the $ATT$. The introduction of these unobservables exacerbates the issue, causing undersized grids to suffer even more. The bottom panel of Figure \@ref(fig:grid-uoa) shows how coverage probability is affected by grid size.

```{r grid-uoa, fig.align="center", fig.cap="bias depends on grid size when property level unobservables are present.", out.width = "65%", fig.pos='H', fig.show='hold'}
knitr::include_graphics(c("figs/bias_gridsize_uoa.png", "figs/cover_gridsize_uoa.png"))
```

These findings further demonstrate the benefits of conducting the analysis at the scale of the relevant decision-making unit. Overall, this section highlights the benefits of structuring one's econometric modeling decisions to match the scale that drives landscape heterogeneity.
