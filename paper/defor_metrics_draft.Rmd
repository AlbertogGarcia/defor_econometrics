...
We now vary the grid cell resolution used in our monte carlo simulations to understand the importance of scale when conducting conservation impact evaluations. Table XX helps us to get a sense of relative scale within our synthetic landscapes for this section. We assume pixels are comparable to Landsat resolution (30 m), and so the average property in our landscape is 81 hectares. 
...
The top panel of Figure \@ref(fig:grid-fe) shows that even in the absence of property-level unobservables ($\sigma_p = 0$), too small of grids result in biased estimates of the $ATT$. The introduction of property unobservables exacerbates the issue with small grid cells, causing undersized grids to result in upward bias much more severe than when property unobservables play a lesser role. For any given value of $\sigma_p$, the middle panel shows that coverage probability is nearest the desired 95\% near the scale of the average property. Also, note that when property unobservables play a significant role and grid size is large, coverage is near 100\%, reflecting large standard errors. Lastly, the bottom panel shows that RMSE tends to approach its minimum near the scale of the average property as well. 
