...
* $HR_1 = exp( \alpha )$, where $\alpha$ is estimated by subsetting to observations from the post-treatment period $(t \geq t_0)$, and estimating the hazard rate of deforestation at time $t$ as $h(t) =\gamma_0(t) exp( \alpha 1\{ D_i = 1\} )$;

* $HR_2 = exp( \beta )$, where $\beta$ is estimated by subsetting to observations from the treated group $(D_i = 1)$, and estimating the hazard rate of deforestation at time $t$ as $h(t) =\lambda_0(t) exp(\beta 1\{ t \geq t_0\} )$; and
...
Because the numerator of $HRTT$, $E[y_{ivt}(1)  |  t\geq t_0, D_i=1]$, can be estimated as the mean of post-treatment deforestation rates in the treated group, we can estimate the $ATT$ using this observable term and our estimate of $HRTT$:

$$\widehat{ATT}_{Cox} = E[y_{ivt}(1)  |  t\geq t_0, D_i=1] - \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1] }{\widehat{HRTT}}$$
Here, we have shown that the simple extension of the traditional DID to the survival setting does not clearly recover an easily interpretable measure of a policy's impact. In contrast, our proposed estimator, which relies on separate estimation of relevant hazard ratios, does recover the relevant analog of the $ATT$. We explore the performance of $\widehat{ATT}_{Cox}$ relative to the proposed OLS regressions under various circumstances likely to arise in the deforestation setting in the next sections. 
...
In light of the fact that the simple Cox DID is not a viable analog to the traditional DID, we explore whether our proposed estimator, recovers relevant treatment effect parameters in the deforestation setting. We see in column 10 that $\widehat{ATT}_{Cox}$ indeed recovers our parameter of interest.
...
While incorporating spatially aggregated units in OLS specifications may improve analysis, there is no clear analog with survival models. In fact, Figure \@ref(fig:summary) in the appendix shows that $\widehat{ATT}_{Cox}$ suffers significantly as $\sigma_p$ increases. As such, when the pixel is not the preferred unit of analysis, survival analysis is not likely to be the preferred method for estimating the impact of conservation interventions.
