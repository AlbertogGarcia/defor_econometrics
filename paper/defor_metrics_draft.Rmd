...
While models in the form of Regression 3 are an intuitive way to generalize the DID to survival analysis, $exp(\beta_{coxDID})$ generally does not recover the $HRTT$. Instead, it recovers a less intuitive product of the two pre- and post-treatment ratios across the two groups:
...
```{r cox-fig1, fig.cap=" bias, distribution, and coverage of Cox proportional hazard DID and our proposed estimator relative to pixel DID using OLS.",  out.width="100%"}
survival_long <- readRDS("results/survival_long.rds")
...
df_summary <- survival_long %>%
  ...
  summarise(RMSE = rmse(bias, 0),
            ...
            Bias = mean(bias),
            cover = mean(cover))%>%
  mutate("cox DID" = as.logical(ifelse(model == "cox interaction", 1, 0)),
         "pixel OLS DID" = as.logical(ifelse(model == "pixel DID", 1, 0)),
        "HE estimator" = as.logical(ifelse(model == "cox did", 1, 0)) )%>%
  filter(model %in% c("cox did", "pixel DID", "cox interaction"))%>%
  select(Bias, everything())%>%
  arrange(RMSE)

df_summary[ 4 , ] <- NA
df_summary <- rbind( df_summary[4,],
  df_summary)

par(oma=c(1,0,1,1))

labels <- list("Estimator" = c("pixel OLS DID", "Cox PH DID", "HE estimator"))

#f <- function(x) gsub("^(\\s*[+|-]?)0\\.", "\\1.", as.character(x))
coverage <- c("", round(df_summary$cover[2:4], digits=3), "")
RMSE <- c("", round(df_summary$RMSE[2:4], digits=5), "")
...
We see that while the Cox DID regression (Regression 3) does not recover the impact of the conservation intervention in this context, $\widehat{HE}$ does. However, there are several reasons why survival analysis may not be preferable in the context of estimating the impacts of conservation interventions on deforestation rates. One of the primary reasons for the use of survival analysis is censoring. This occurs when the researcher has partial information about the subjects’ survival times but does not have access to precise event times. While the researcher may not observe all pixels until they are deforested, mayn other forms of censoring are not a concern in the context of deforestation. Further, the proposed strategy to drop pixels in the periods after they are first deforested successfully addresses the irreversibility in deforestation events. As we will show in the next sections, when the process of deforestation operates at a scale different than the pixel, an alternatiive unit of analysis may be preferable. This is not feasible in survival analysis in the way it is in our alternative OLS specifications. We explore how some of these factors influence the relative performance of various models in the following sections. 
...
```{r selection-fig, results = FALSE, warning = FALSE, message = FALSE, fig.cap="Non-random selection introduces bias into estimates. Specifications are ordered by mean bias (least to most). ",  fig.width = 9, fig.height = 5, out.width="100%"}
...
The introduction of $\sigma_p$ changes the relative performance of each specification. Figure XX shows that the pixel-level DID begins to suffer in terms of bias, RMSE, and coverage as these property-level unobservables play a larger role in the data generating process. 

```{r did-property, results = FALSE, message = FALSE, warning = FALSE, fig.align="center", fig.cap="bias, distribution, and coverage of pixel-level DID as property level disturbances play larger role in the data generating process", fig.width = 10, fig.height = 8, out.width = "100%", fig.pos='H', fig.show='hold'}

#### issue of disturbances on DID

full_summary <- readRDS("results/summary_full.rds") %>%
  filter(treatment.fe == 1 & se_pixel == 1 & is.na(notes)) %>%
  mutate_at(vars(bias, cover), as.numeric)%>%
  group_by(std_p)%>%
...
RMSE_print<- round(as.numeric(RMSE), digits =3)
RMSE_print[is.na(RMSE_print)] <- " "

schart_results <- select_results %>% 
  select(c(Bias, s0, s1, s2, s3, q05, q95))
...
topline = -0.025
midline = topline-0.02
ylim <- c(midline-0.02,0.01)
...
schart(as.data.frame(schart_results), labels = labels, ylim = ylim, axes = FALSE, index.ci=index.ci, ylab="", 
       ...
       heights = c(8,8), cex = c(1.75,1.75)
      )

mtext("Bias     ", side=2, at = -0.005, font=2, las=1, line=.5, cex = 2)
Axis(side=2, at = c(-0.01, 0, 0.01), labels=TRUE, cex = 3)
abline(h=0, lty=2, lwd = 2)
abline(h=topline)
abline(h=midline)
lapply(1:length(RMSE), function(i) {
  text(x= i, y=midline+0.01, paste0(RMSE_print[i]), col="black", font=1, cex=2)
  text(x= i, y=midline-0.01, paste0(c_print[i]), col="black", font=1, cex=2)
})
mtext("RMSE", side=2, at = midline+0.01, font=2, las=1, line=.5, cex = 2)
mtext("Coverage\nprobability", side=2, at = midline-0.01, font=2, las=1, line=.5, cex = 2)


```

In Figure XX, we see that incorporating spatially aggregated units reduces bias relative to the simple pixel-level DID when the property influence is relatively large ($\sigma_p= 0.5$). This improvement is apparent in both specifications incorporating spatially aggregated units as the level of unit fixed effects (Regression 4) or as the unit of analysis (Regression 5). Further, we see that the scale of aggregation plays a role. Analyses incorporating the property or grid cells near the size of the average property outperform models utilizing larger or smaller scales. Ultimately, all models that incorporate spatial aggregation suffer from relatively little bias and incur less bias than the simple pixel-level DID. In Appendix \@ref(property-alt), we see that specifications incorporating the property tend to outperform other models in alternative parameterizations as well. 

```{r agg-property, results = FALSE, message = FALSE, warning = FALSE, fig.align="center", fig.cap="bias, distribution, and coverage with $\\sigma_p=0.5$ for Left: specifications with aggregated unit fixed effects; and Right: specifications with aggregatiod units of analysis", fig.width = 10, fig.height = 8, out.width = "100%", fig.pos='H', fig.show='hold'}

full_summary <- readRDS("results/summary_full.rds") %>%
  mutate_at(vars(bias, cover), as.numeric)%>%
  filter(std_p==0.5 & is.na(notes) & pixel.fe == 0 & weights == 0)%>%
...
text(x=41
     , y=.0475, expression(paste(sigma[p],"=0.5")), col="black", font=2, cex = 1.1)
...
## Cox proportional hazards difference-in-differences {#coxph}
