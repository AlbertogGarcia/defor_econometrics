...
To do:
\begin{itemize}
\item figure captions/legends (did labels, twfe colors, year and rate graphs)
\item citations
\item characteristics of pixel level satellite data that create bias
\item property level unobservable section
\item functional form graph and discussion
\item why we settled on the decisions we did and move more technical discussion to appendix
\item advice and key takeaways section
...
```{r, echo=FALSE,out.width="49%", fig.cap="Difference-in-Differences intuition", fig.show='hold', fig.align='center'}
knitr::include_graphics(c("figs/trends.png"))#, "landscape_figs/landscape_did.png"))
```
...
This bias is unrelated to that which may arise from satellite sensor characteristics, satellite angle, or atmospheric conditions [@jain2020]. We do not explore the interaction of these two potential sources of bias in our paper, however, it is likely an important consideration.  
...
We explore the effect of using non-linear functional forms. We use DID regression models at the pixel level in this section to avoid the aforementioned issues with pixel level TWFE models. Coefficient estimates are converted into the $ATT$ using the \textbf{mfx} package in R, which takes into account the binary nature of the treatment variable. We also explore how functional form choice may depend on the deforestation rates in a given setting, sice functional form choice may have more of an impact in low event probability scenarios. 
...
The underlying DGP and error structure play a role in determining which functional form leads to the least bias in a given scenario. Alternative parameterizations may affect which functional form is optimal in a given circumstance. Therefore, authors, regardless of the preferred specification, should justify their choice. As is common in the literature already, we advise that authors ensure the result is robust to different choices of functional form. 
...
As seen in Figure 7, outcome 1 results in the least bias in our guiding example. The other outcomes result in relatively greater bias, although this difference seems minimal. 

We express concern surrounding the use of the initial observed baseline forest cover in the calculation of deforestation rates, as in formula (2). As the rate is calculated in periods further away from the baseline, the deforestation rate will be increasingly high. This may impact the viability of the analysis, as the rate becomes artificially inflated in later years and differentially affect the calculation in groups with different baseline rates. We explore this issue in the Appendix, and indeed, formula (2) is the least robust to changes in the length of observation period. As the observation period grows, formula (2) progressively under(over)estimates deforestation rates in the group with a lower baseline deforestation rate relative to the other group. 

<!-- Albert: Do you think the exercise in the appendix should be brought to the main text? -->
...
kable(coverage_perr[,2:7], format = "latex", row.names = FALSE, 
      ...
      col.names = NULL) %>% 
  add_header_above(c("unit of analysis"=1, "standard error structure"=1, "coverage probability" = 4), escape = FALSE, line =TRUE, bold = TRUE)%>%
  add_header_above(c(" "=1, " "=1, "$\\sigma_p = 0$"=1, "$\\sigma_p = 0.05$" = 1, "$\\sigma_p = 0.15$" = 1, "$\\sigma_p = 0.25$" = 1), escape = FALSE, line =TRUE, bold =TRUE)%>%
  kable_styling(font_size = 10, latex_options = c("HOLD_position"),bootstrap_options = c("striped", "hover", "condensed", "responsive"),#full_width = TRUE
                position = "center")
...
=& P(y^*_{it}(0) > 0 |  t \geq t_0, D_i=0) - P(y^*_{it}(0)>0 |  t<t_0, D_i=0)\\
=& P(-\alpha_i -u_{it} < \beta_0 +\beta_1+\beta_2) - P(-\alpha_i -u_{it} < \beta_0+\beta_1 )\\
=& F(\beta_0 ++\beta_1+ \beta_2) - F(\beta_0+\beta_1)
\end{align*}
...
\begin{align*}
&trend= F(\beta_0 + \beta_{20} ) - F(\beta_0) \\
\Leftrightarrow \\
&trend + baseline_0 =F( \beta_0 + \beta_{20})\\
\Leftrightarrow \\
&F^{-1}(trend + baseline_0 ) =\beta_0 + \beta_{20}\\
\Leftrightarrow \\
...
\begin{align*}
&trend= F(\beta_0 + \beta_{1}+\beta_{21} ) - F(\beta_0 + \beta_1) \\
\Leftrightarrow \\
&trend + baseline_1 =F( \beta_0 +\beta_1+ \beta_{21})\\
\Leftrightarrow \\
&F^{-1}(trend + baseline_1 ) =\beta_0 + +\beta_1+ \beta_{21}\\
\Leftrightarrow \\
...
solving for $\beta_3$
\begin{align*}
&ATT= F(\beta_0 + \beta_1 +\beta_{21} +\beta_3) - F(\beta_0 + \beta_1 +\beta_{21})\\
\Leftrightarrow \\
&ATT + F(\beta_0 + \beta_1 +\beta_{21}) = F(\beta_0 + \beta_1 +\beta_{21} +\beta_3) \\
\Leftrightarrow \\
&F^{-1}(ATT + F(\beta_0 + \beta_1 +\beta_{21}) )= \beta_0 + \beta_1 +\beta_{21} +\beta_3\\
\Leftrightarrow \\
&\beta_3 = F^{-1}(ATT + F(\beta_0 + \beta_1 +\beta_{21}) )- (\beta_0 + \beta_1 +\beta_{21})\\
\end{align*}
