...
The top panel of Figure \@ref(fig:grid-size) shows the bias in pixel-level TWFE regression with grid unit fixed effects (Regression 3, where $j$ represents grid cell $j$). Note that even in the absence of property-level unobservables ($\sigma_p = 0$), too small of grids result in biased estimates of the $ATT$. The introduction of these unobservables exacerbates the issue, making undersized grids result in even more biased estimates. Bias is decreasing in grid cell size, and it is minimized when the grid cell is near the scale of the property. The bottom panel of Figure \@ref(fig:grid-size) shows the bias in grid-level TWFE regressions (Regression 4, where $j$ represents grid cell $j$). We see that grid cell size plays a minimal role in the absence of property level unobservables. When these unobservables are present, however, grid cell resolution becomes more important. While grid sizes nearest to the average property do not minimize bias in this case, if grid cell size is too small or large, bias becomes more severe. (Note that grid cell size was larger than average property size in the section above). These findings further demonstrate the benefits of conducting the analysis at the scale of the relevant decision-making unit. Overall, this section highlights the benefits of structuring one's econometric modeling decisions to match the scale that drives landscape heterogeneity.
