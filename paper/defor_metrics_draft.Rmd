...
We find little evidence that any one aggregated unit of analysis is consistently preferrable in terms of bias. Columns 3-5 of Figure \@ref(fig:summary-fig) shows that the bias of the estimates is not critically different depending on level of aggregation, however, the distributions are varied. In particular, the distribution of estimates was widest with property level aggregation due to the heterogeneity in property sizes across the landscape. 
...
One can also use fixed effects at the level of an administrative unit rather than the pixel level to resolve bias associated with pixel level TWFE regressions. Columns 6-8 of Figure \@ref(fig:summary-fig) show that pixel level TWFE regressions are unbiased estimators of the $ATT$ when grid, county, or property fixed effects are used rather than pixel fixed effects. We also see that in the absence of property level perturbations (i.e. $\sigma_p=0$ in the DGP), all three models provide nearly identical estimates. The estimate distributions are similar across all three models. 
...
The introduction of property level unobservables changes the relative performance of each level of aggregation. Figure \@ref(fig:property-uoa) demonstrates that models incorporating the property into the analysis, whether with aggregated units of analysis or fixed effects, perform relatively better in terms of bias when the standard error governing the property level disturbances, $\sigma_p$, increases. The left panel of Figure \@ref(fig:property-uoa) demonstrates that models with the property as the unit of analysis (orange) outperform models with any other unit of analysis. The pixel level DID suffers from the highest levels of bias regardless of the property level unobservables. The right panel of Figure \@ref(fig:property-uoa) demonstrates a similar takeaway in terms of aggregating fixed effects. We see that models with grid, property, or county fixed effects perform similarly in terms of bias when there are no property level disturbances. However, as $\sigma_p$ increases, the model with property fixed effects outperforms the others. These findings indicate that incorporating the relevant decision making unit into the econometric specification reduces estimate bias. 
...
```{r, fig.cap="bias, distribution, and coverage of all models with standard errors clustered at the level of the model fixed effects. The pixel level DID is clustered at the pixel level. ", out.width = "100%", fig.pos='H', fig.show='hold'}
