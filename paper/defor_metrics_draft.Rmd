...
## Modeling deforestation pressure
...
Researchers generally cannot observe $y^*_{ivt}$ directly. Instead, they observe maps depicting pixel-level, binary deforestation, similar to what is depicted in Figure \@ref(fig:counterfactual). In the form of a panel dataset, the researcher observes the year in which a pixel is converted from forest to non-forest, and importantly, deforested pixels do not revert back to forest cover. Thus, repeated deforestation events cannot be detected in the same location. In order to correct for this irreversibility, many studies drop deforested pixels in the periods after they first become deforested [@jones2015; @alix-garcia2017]. In the literature, the outcome variable describing pixel $i$, in property $v$, in time $t$ is therefore defined as follows:
\begin{align*}
...
Some researchers opt to keep deforested pixels in the panel, and instead of estimating a program's impact on deforestation rates, claim to estimate the programs impact on deforested area. However, when reforestation timing is unaccounted for, as is the case with most deforestation data products, this is not possible. Rather than measuring deforested area at any given time, this outcome variable measures the stock of ever-deforested area through the current time period. It is unclear that this is a relevant parameter of interest in many cases and should not be interpreted as either a deforestation rate or the deforested area in a given time period. See (Appendix \@ref(keep-pixels)) for further discussion on this point.
...
In order to see how this non-random selection influences estimates in our simulated setting, we set $\sigma_a$, the standard error of the time-invariant pixel-level disturbances equal to $0.01$. Figure \@ref(fig:selection-fig) shows that non-random selection introduces a slight downward bias across every specification (specifications ordered from least to most bias). Pixel-level specifications suffer slightly more than specifications with a spatially aggregated unit of analysis in terms of bias and coverage. We note that in practice, researchers cannot recover the second and fourth terms of the bias term in equation 10, meaning that the magnitude of this bias is unknown to the researcher. However, this bias is likely to be of a smaller magnitude in reality than in our simulated setting, since our simulated time frame for forest regrowth is likely to short to reflect true reforestation timing. For the remainder of the paper, we remove the bias due to non-random selection (only possible in the simulation setting). 
...
<!-- Largely, I changed the discussion from reflecting the decision-making unit as the preferred scale to just accounting for the spatial process - since property level aggregation was no longer the clear front runner after accounting for the selection  -->
...
## indroducing property level unobservables

Property level unobservables impact both treatment effect estimates and coverage probabilities. This is likely to be a factor when land use decisions are made at the property level, or the intervention seeks to alter underlying landowner incentives. Characteristics of land are also likely to be similar within the same spatial locations. In our Monte Carlo simulations, we vary the standard deviation of the time-invariant property level disturbances in the initial DGP, $\sigma_p$, in order to account for these unobservables. 
...
<!-- Even accounting for the non-random selection, there still seems to be a slight downward bias. Unclear if this is actually remaining bias or if I'm undercounting the bias from non-random selection -->
...
Authors frequently choose to use a pixel or grid cell as their preferred unit of analysis for purposes of interpretation. The coefficient of interest from a specification using the pixel as the unit of analysis can be interpreted as the effect of the intervention across the landscape. In contrast, if a property is used, the coefficient should be interpreted as the effect of the intervention on the characteristic property in the sample. In order to reobtain a landscape-scale interpretation, one must weight the regression by the area of the unit of analysis (i.e. property). In our case, weighting does not seem to have a major impact on bias, RMSE, or coverage probability. It should be noted that the use of area weights is likely to be most useful when the $ATT$ for the characteristic property differs from the $ATT$ across the landscape. In our current framework, these are the same, making it dificult to highlight to potential benefits of area weights. We plan to extend the paper on this front. 


<!-- Probably should try to find citation for above section -->

## Matching grid cell resolution to scale of heterogeneity may reduce bias

In some instances, researchers do not have access to geospatial boundaries of these relevant administrative units. In these cases, utilizing a grid cell or alternative administrative unit to capture some spatial heterogeneity is likely preferable to pixel level DID models for purposes of both estimation and inference. The scale of this grid cell or alternative unit relative to the process of land use change is important. Figure \@ref(fig:grid-size) shows that grid cell size plays a role when property level unobservables are present ($\sigma_p = 0$ or  $0.2$). 

```{r grid-size, fig.align="center", fig.cap="bias depends on grid size when property level unobservables are present.", out.width = "65%", fig.pos='H', fig.show='hold'}
knitr::include_graphics(c("figs/gridsize_fe.png", "figs/gridsize_uoa.png"))
```

The top panel of Figure \@ref(fig:grid-size) shows the bias in pixel-level TWFE regression with grid unit fixed effects (Regression 3, where $j$ represents grid cell $j$). Note that even in the absence of property-level unobservables ($\sigma_p = 0$), too small of grids result in biased estimates of the $ATT$. The introduction of these unobservables exacerbates the issue. Bias is decreasing in grid cell size, and it is minimized when the grid cell is near the scale of the property. The bottom panel of Figure \@ref(fig:grid-size) shows the bias in grid-level TWFE regressions (Regression 4, where $j$ represents grid cell $j$). We see that grid cell size plays a minimal role in the absence of property level unobservables. When these unobservables are present, however, grid cell resolution becomes more important. While grid sizes nearest to the average property do not minimize bias in this case, if grid cell size is too small or large, bias becomes more severe. (Note that grid cell size was larger than average property size in the section above). These findings further demonstrate the benefits of conducting the analysis at the scale of the relevant decision-making unit. Overall, this section highlights the benefits of structuring one's econometric modeling decisions to match the scale that drives landscape heterogeneity.
...
Here, we introduce a setting in which groups of units receive treatment at different times (full DGP can be found in Appendix ...). We consider three groups: an early group, a late group, and a never-treated group. Each group experiences differing pre-treatment deforestation rates (7\%, 4\%, and 2\% for the early, late, and never-treated groups, respectively) and no time trend. The on-impact $ATT$ for the early group is $-0.02$ for both treated groups. Common trends is satisfied by construction, and we do not introduce any dynamic effects. Figure \@ref(fig:observed-multiGT) shows the observed deforestation rates ($E[y^o_{ivt}]$) from one iteration of our simulation in this setting. 
...
<!-- ## Evidence of weighting issues -->

<!-- Figure \@ref(fig:multiGT2) examines the performance of the @callaway2020 and @gardner2021 estimators relative to TWFE regressions when treatment effects vary across time and across groups. The parameterization and DGP can be found in Appendix ... Figure XX shows deforestation rates in each of the three groups through time, and Figure XX shows the event study estimates produced by each of the three estimators as well as the "truth". We see that the @callaway2020 and @gardner2021 estimators slightly outperform the TWFE estimator. This is evidence of the weighting that has become a concern with TWFE estimators in these type of settings. While each estimator produces a weighted average of group-time treatment effects, the weights do not neccessarily correspond to each groups representation in the sample as do the weights in the alternative estimators.  -->

<!-- ```{r multiGT2, fig.cap="New estimators, similar to TWFE regressions with pixel unit fixed effects, cannot identify ATT with pixel as unit of analysis", fig.pos='H', fig.show='hold'} -->
<!-- knitr::include_graphics(c("figs/county_multiGT2.png")) -->
<!-- ``` -->
