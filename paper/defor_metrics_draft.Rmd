...
Robert comment: If we do target Conservation Letters, maybe we move the technical discussion of these elements to an appendix? Just have 1-3 paragraphs that highlight that these are critical choices that the researcher needs to make?
-->
...
<!-- Robert: Maybe its worth noting that we're assuming "trend" is applied to both treatment and control units, and that we're assuming this trend is equal across the two groups (parallel trends) -->
...
\end{align}
The $\beta$ coefficients can be defined as functions of the four parameters assigned by the researcher ($baseline_0$, $baseline_1$, $trend$, $ATT$, derivation provided in Appendix 1). A pixel-specific random disturbance is generated according to $\alpha_i \sim N(0, \sigma_a^2)$ and the error term is generated according to $u_{it} \sim N(0, \sigma_u^2)$. 
