...
<!-- To do: -->
<!-- - figure captions/legends (did labels annd brackets, legends, and figure 2 alignment) -->
<!-- - citations -->
...
```{r, echo=FALSE,out.width="75%", fig.pos = 'H', fig.cap="This figure shows deforestation that would have ocurred in the absence of the conservation intervention at the same point in time, after implementation of the conservation intervention. Note that no counterfactual deforestation occurs in the control area, since no intervention ocurred.", fig.show='hold', fig.align='center'}
...
Figure 2 shows the intuition behind the DID estimator in two time periods. Deforestation rates in the control area changed between the first and second period (blue line). The common trends assumption amounts to assuming that the deforestation rate in the intervention area would have followed the same trajectory as the rate in the untreated area had the intervention not ocurred (dashed line). The DID estimator ("represented by the red bracket"estimated $ATT$" in Figure 2) then amounts to the difference in pre-treatment deforestation rates ("pre-treatment difference" in Figure 2) minus the post-treatment difference in deforestation rates ("post-treatment difference" in Figure 2) between units with and without the intervention.
<!-- Robert: Edit this paragraph -->

```{r, echo=FALSE,out.width="49%", fig.cap="Difference-in-Differences intuition in two time periods", fig.show='hold', fig.pos = 'H'}
...
kable(lit_table, format = "latex", 
      caption = "Table of common model decisions in the literature",
      col.names = c("Paper",
                    "Panel Method",
                    "Unit of analysis",
                    "Functional form")) %>% 
...
Analyses at the pixel level are prevalent in the literature, as seen in Table 1. Further, it has often been promoted as the preferred unit of analysis in many cases. To account for the fact that deforested pixels generally do not revert back to forest cover over the course of the study period, it has been advised to drop deforested pixels in the periods after they first become deforested [@jones2015; @alix-garcia2017]. We show in the Appendix (7.1) that regression models failing to drop deforested pixels in subsequent periods do indeed incur severe bias. 

Despite widespread use of pixel level analyses, they are problematic in the context of TWFE regression models. In fact, the TWFE model yields the post-treatment difference in outcomes (single difference), rather than the desired ATT. We provide two forms of evidence to support this claim: (1) evidence from our Monte Carlo simulations and (2) an analytical proof found in the appendix (7.2). The result stems from the fact that the TWFE regression is not able to identify off of pixels that are dropped in the first period. Thus the pre-treatment period deforestation rates are not accounted for in the TWFE estimates. 
...
We explore the bias, RMSE, and coverage probabilities corresponding to different levels of aggregation in the context of our guiding example. We find little evidence that any one level of aggregation is consistently preferrable in terms of bias, meaning that RMSE and coverage probability play a larger role as an evaluation metric. Figure 5 shows that the bias of the estimates is not critically different depending on level of aggregation, however, the distributions are varied. RMSE is highest with property level aggregation due to the heterogeneity in property sizes across the landscape. 

Coverage probabilities are impacted by the distribution of the estimates, bias, and standard error structure. We compare coverage at each level of aggregation in Table 2. We have not yet introduced any structure into the DGP that would warrant clustering, so performance is based on classical standard errors. This is demonstrated by coverage probabilities near 95\% in the absence of any property level unobservables ($\sigma_p = 0$) for all levels of aggregation. Despite the clear differences in the distributions of estimates, both pixel and property-level analyses yield the desired 95\% coverage probabilities. When the desired coverage probability is achieved, it is preferable to use models with a lower RMSE. 
...
Property level unobservables may impact both treatment effect estimates and coverage probabilities. This is likely to be a factor when land use decisions are made at the property level or the intervention seeks to alter underlying landowner incentives. The error term in the initial DGP, $\rho_v$, varies at the property level in order to account for these unobservables. We explore the cases when the property level disturbances are generated according to $\rho_v \sim N(0, \sigma_p^2)$ with $\sigma_p = 0.00$, $0.05$, $0.15$, and $0.25$. 
...
```{r, echo=FALSE,out.width="49%", fig.cap="Performance of different levels of aggregation in the presence of property level unbservables Left: bias as a function of $\\sigma_p$ by level of aggregation; Right: RMSE as a function of $\\sigma_p$ by level of aggregation", fig.pos='H', fig.show='hold',fig.align='center'}
...
Upon examining Table 3, we see that clustering to the respective level of aggregation does indeed improve the coverage probability in the presence of property level unobservables. Property level aggregation leads to preferred coverage probabilities, regardless of the standard error structure. When using the pixel as the unit of analysis, clustering standard errors at the property level leads to improved coverage probability, however, as mentioned above, the pixel level analyses suffer from increased bias and RMSE. These results highlight the benefit of using the relevant decision-making unit as the unit of analysis as well as clustering standard errors at that level. 
...
## Functional Form
Given that all of the outcome metrics used in the studies presented in Table 1 are generated from maps of binary, pixel-level deforestation events, several researchers have suggested that non-linear functional forms may be preferred over linear models. For example, models estimated using individual pixels may be better represented by a probit or logit functional form (XX - Add refs), whereas models that count deforested pixels within a broader unit of observation (i.e. grid cell) may be best modeled using a Poisson distribution [@burgess2012; @busch2015a; @carlson2018]. However, these non-linear functional forms can complicate interpretation of regression coefficients [@gomila2019]. We explore whether the use of non-linear functional forms reduce bias in ATT estimates. We find that, in many cases, a simple Linear Probability Model (LPM) results in less bias than alternate, non-linear functional forms. For example, Figure 7 shows the distribution of model bias resulting from different model structures in our guiding parameterization. 

```{r, echo=FALSE,out.width="49%", fig.cap="Distribution of estimates produced by different functional forms. We use DID regression models at the pixel level in this section to avoid the aforementioned issues with pixel level TWFE models. Left: distribution of estimate bias from linear probability model, logit, and probit models with binary unit of analysis; Right: distribution of estimate bias from grid level linear and poisson models", fig.pos='H', fig.show='hold',fig.align='center'}
...
Albert: the results indicate that we want to focus on the trend line, however, I think it might be important to see what is happening with low deforestation rates, since people might expect that to be an instance when functional form is important
-->

The underlying DGP and error structure play a role in determining which functional form leads to the least bias in a given scenario. Alternative parameterizations may affect which functional form is optimal in a given circumstance. Further, one can imagine a slightly different DGP in which the preferred functional form is different than in our setup. Therefore, authors, regardless of the preferred specification, should justify their choice, taking into account the DGP underlying their specific context. We also recommend that authors ensure the result is robust to alternate functional forms. 
...
<!-- Although we do not explore grid cell or property size/weighting at this point, it woulld be good to give some frame of referrence. I'm thinking we pretend each pixel is 30x30 and then determine how many pixels each property/grid/etc. are made of -->
