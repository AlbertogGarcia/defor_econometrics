...
While survival models have not frequently been used in the context of avoided deforestation, several other literatures have used them in settings with irreversible outcomes of interest such as mortality (e.g., @friedman) or recidivism (e.g., @mastrobuoni). Here, the researcher is interested in the time each unit remains in the sample, which in our context, corresponds to the time each pixel survives until deforestation. While this class of models cannot estimate the $ATT$ directly, the hazard ratio analog is of similar interest:

$$ HRR_{ATT} = \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=1]}$$

The $HRR_{ATT}$ is the ratio of the hazard rates in the treatment group post-treatment, relative to the counterfactual in that group had treatment not ocurred. Because the numerator of $HRR_{ATT}$ is observable, we can recover the $ATT$ from the observable term and $HRR_{ATT}$:

$$ATT = E[y_{ivt}(1)  |  t\geq t_0, D_i=1] - \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1] }{HRR_{ATT}}$$

In settings with two groups and two time periods, some researchers opt to estimate $HRR$ with a cox proportional hazards model of the following form:
...
, where $h(t)$ is the hazard rate of deforestation, $t$ years into the study period; and $\delta_0(t)$ is the baseline hazard function. Then, the expectation is that $exp(\beta_{coxDID}) = HRR_{ATT}$.

### Proposing an alternative survival estimator

While models in the form of Regression 3 are an intuitive way to generalize the DID to survival analysis, $exp(\beta_{coxDID})$ generally does not recover the $HRR_{ATT}$. Instead,

$$\beta_{coxDID} = \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]E[y_{ivt}(0)  |  t < t_0, D_i=0] }{E[y_{ivt}(0)  |  t\geq t_0, D_i=0]E[y_{ivt}(0)  |  t < t_0, D_i=1] }$$

We propose an alternate estimator to recover the $HRR_{ATT}$ using three separate hazard rate ratios: 
...
We explore whether survival models, including our proposed estimator, are a viable approach to recover treatment effects in the deforestation setting. 
