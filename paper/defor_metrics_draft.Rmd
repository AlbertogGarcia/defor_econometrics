...
The introduction of property level unobservables changes the relative performance of each level of aggregation in terms of bias, RMSE, and coverage. Models incorporating the property into the analysis, whether with aggregated units of analysis or fixed effects, perform relatively better in terms of bias when the standard error governing the property level disturbances, $\sigma_p$, increases. Figure \@ref(fig:property-uoa) demonstrates that models with the property as the unit of analysis (orange) outperform models with any other unit of analysis. The pixel level DID suffers from the highest levels of bias regardless of the property level unobservables. 

These findings indicate that incorporating the relevant decision making unit into the econometric specification reduces estimate bias. 
...
The right panel of Figure \@ref(fig:property-uoa) demonstrates a similar takeaway in terms of aggregating fixed effects. We see that models with grid, property, or county fixed effects perform similarly in terms of bias when there are no property level disturbances. However, as $\sigma_p$ increases, the model with property fixed effects outperforms the others. 



<!-- Implications for RMSE -->

<!-- RMSE is also impacted in the presence of property level unobservables. Again, pixel level analyses suffer the most from increased RMSE as property level unobservables become more of a factor. This is partially due to the change in bias.  -->
...
coverage_did <- read.csv("did_coverage.csv")[2:7] #%>%
...
Table \@ref(tab:did-cover) demonstrates how the presence of property level unobservables impacts coverage probability in the pixel level DID model. Clustering standard errors clearly has an impact on coverage probability as well as the level at which they are clustered. The issue with the use of classical standard errors becomes apparent when property level unobservables enter the DGP, as evidence by the last row of Table XX. Clustering standard errors improves coverage probability with clustering at the county and property level outperforming the grid level.
...
Table XX and Table XX demonstrate how coverage probabilities are impacted by property level unobservables for TWFE models with aggregated units of analysis and fixed effects, respectively. While the property level specification incurred the least bias with the introduction of property level disturbances, Table XX shows that both the property and county level models hover near the desired 95\% coverage probabilities as the property level disturbances become a factor. On the other hand, the grid level model's coverage probability declines as the disturbances play a larger role. 

The pixel level model with property fixed effects incurred less bias than the pixel level models with other alternative fixed effects. Table XX shows that the model with property fixed effects also outperforms the other models in this table in terms of coverage probability. Even as $]sigma_p$ grows, the property fixed effects model maintains a coverage probability near the desired 95\%. 

These results highlight the importance of taking the decision-making process underlying deforestation into account. 

In many instances, researchers do not have access to geospatial boundaries of the relevant decision-making unit. County and grid cell level aggregation lead to lower levels of bias and higher coverage probability than pixel level specifications when property unobservables are present. County level aggregation leads to the best coverage in the absence of property boundaries, meaning that a larger, alternative administrative unit may be the best alternative when an adequate number of observations are available. If a pixel level analysis is used (potentially due to constraints on geospatial boundary access or few administrative units), we see that coverage is greatly improved when standard errors are clustered to a higher unit of aggregation.  
...
knitr::include_graphics("figs/summary_full.png")
```
...
This paper seeks to leave researchers with a clear sense of the decisions that play a role in their choice of econometric specification. Despite past guidance to the contrary, models that incorporate fixed effects for individual points are unable to yield unbiased estimates of a policy's $ATT$ when applied to irreversible, binary observations of deforestation. This is due to the fact that when pixels are dropped after they first become deforested (necessary in the case of an irreversible binary outcome), TWFE models cannot identify off of points deforested in the first period. This main finding casts doubt as to the reliability of a significant number of estimates in the avoided deforestation literature. 

In order to avoid the issue associated with TWFE models that incorporate point fixed effects, researchers can take advantage of aggregated spatial units. TWFE models with aggregated units of analysis and point level TWFE models with aggregated fixed effects are both viable solutions. We explore the benefits of conducting the analysis and inference at the level of the relevant decision making unit and offer guidance for when it is not relevant or available. Bias can arise when the scale of the analysis differs from the scale at which the process of land use change occurs. We provide guidance for when the researcher does not have access to geospatial boundaries detailing the relevant administrative or decision-making units. Ultimately, context plays a role in all of these decisions, and researchers should make clear which decisions they have made as well as their reasoning. 
