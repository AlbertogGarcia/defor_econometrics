...
labels <- list("Unit of analysis:" = c("pixel", "grid", "property", "county"),
               ...
               "Survival" = c("Cox PH", "ATT cox estimator"),
...
* $ATT_{ls} = \frac{1}{n_{i:D_i=1}}\sum_{i:D_i=1}(y_{ivt}(1) - y_{ivt}(0))$, where $n_{i:D_i=1}$ is the number of treated pixels in the simulated landscape; and

* $ATT_{property} = \frac{1}{n_{v:D_v=1}} \sum_{v:D_v=1}(\frac{1}{n_{iv}} \sum_{i=1}^{n_{iv}}(y_{ivt}(1) - y_{ivt}(0)))$, where $n_{v:D_v=1}$ is the number of treated properties in the simulated landscape; and $n_{iv}$ is the number of pixels in property $v$.

Because the treatment is more impactful in properties of a smaller size, the treatment effect for the average property is less (greater magnitude) than that across the landscape. Figure \@ref(fig:pweight) shows the sample $ATT$s for both the property and landscape. The property-level TWFE regression identifies the $ATT$ relative to the characteristic property when area weights are not used and the landscape scale $ATT$ when they are used. Researchers should use these area weights when they are interested in the impact of the intervention across the landscape. In cases where the researcher is interested in how an intervention affects incentives at the property level, using these weights may not be necessary. 
...
```{r multiGT, results = FALSE, warning = FALSE, out.width="100%", fig.cap="New estimators, similar to TWFE regressions with pixel unit fixed effects, cannot identify ATT with pixel as unit of analysis", fig.pos='H', fig.width = 10, fig.height = 6, fig.show='hold'}
