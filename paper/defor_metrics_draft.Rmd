...
## Estimating the $ATT$ using survival analysis
### Hazard rate ratios from a single survival model do not estimate the $ATT$
...
Multiple studies across a wide variety of settings have interpreted the resulting exponentietated coefficient $exp(\beta_{coxDID})$ as a hazard ratio that is indicative of the impact that treatment has had on the relative likelihood of survival. Specifically, based on the way this hazard ratio is interpreted in multiple papers, it appears that many researchers expect this hazard ratio to represent the ratio of the hazard rates in the treatment group post-treatment, relative to the counterfactual in that group had treatment not ocurred. This desired hazard ratio $(HRTT)$ can be considered a reframing of the traditional $ATT$ as a ratio rather than a difference: 

$$ HRTT = \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=1]}$$

However, Appendix XX shows that $exp(\beta_{coxDID})$ is actually an estimate of a less intuitive hazard ratio that is the product of pre- and post-treatment ratios across the treated and untreated groups:
...
To the best of our knowledge, no prior studies have successfully combined the Cox Proportional Hazards model and the difference in differences research design to recover an unbiased estimate of the $ATT$. Here we outline a new estimation approach that first recovers an unbiased estimate of the $HRTT$, and then translates this into an estimate of the $ATT$. First, we note that the desired $HRTT$ can be re-written as a combination of three different hazard ratios: 

\begin{align*}
HR_1 &= \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=0]}\\
HR_2 &= \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t < t_0, D_i=1]}\\
...
Each of the three hazard ratios, $HRR_1$, $HRR_2$, and $HRR_3$, can be estimated through separate Cox Proportional Hazards models estimated on subsets of the larger dataset. Specifically:

* $HRR_1$: 

$$h(t) =\gamma_0(t) exp( \alpha 1\{ D_i = 1\} ),$$ subsetting to only the post period;
$$h(t) =\lambda_0(t) exp(\beta 1\{ t \geq t_0\} ),$$ subsetting to only the treated group; and
