...
TWFE regression models are often used to apply DID methods to multiple groups or treatment periods. This generally amounts to estimating a regression that controls for unit and time fixed effects. Intuitively, this can be thought of as including a dummy variable for each unit of analysis and each time period. The fixed effects account for any unobservable confounding variables that may vary across units or through time. When the treatment effect is constant across groups and over time, TWFE regressions estimate the average treatment effect in treated units under the standard common trends assumption [@dechaisemartin2020]. 

Because TWFE regression models are often used to generalize the DID method, they are used in a wider variety of settings and have gained paramount popularity in the land use change impact evaluation literature. Settings in which units undergo treatment in more than two distinct time periods may be amenable to TWFE regression, but not the standard DID method. For example, a researcher may use an TWFE regression model to examine the effectiveness of a network of protected areas, where the protected areas where created at different times or a payment for ecosystem services (PES) program, which enrolls properties in annual cohorts. In the case of two groups and two time periods, the TWFE regression should give an estimate equivalent to that of the DID model [@wooldridge]. This flexibility has led to TWFE regression models becoming the gold standard in the literature. While we do not consider Table \@ref(tab:table-lit) a perfect representation of all deforestation impact evaluation studies using panel data, 17 of the 28 (60.7\%) identified studies use TWFE models. 


# Pixel level, two way fixed effects models fail to estimate the $ATT$

Despite widespread use of pixel level analyses of deforestation, the application of TWFE models to a binary, irreversible process such as deforestation is problematic.

<!--
TODO: add back a short description of why irreversibility necessitates dropping observations. Text is currently in section 2.1
-->

While dropping pixels has been offered as a correction to account for the irreversibility of deforestation events, it is quite problematic. In fact, with this correction, the TWFE model yields the post-treatment difference in outcomes (single difference), rather than the desired $ATT$. We provide an analytical proof to support our claim (Appendix 7.1). The result stems from the fact that the TWFE regression is not able to identify off of pixels that are dropped in the first period. Thus the pre-treatment period deforestation rates are not accounted for in the TWFE estimates. 

The following pixel level TWFE regression is commonly used to estimate a policy's impact on deforestation. It has been used multiple times in studies published in economics journals, with $\beta$ used to identify the $ATT$:
...
Rather than directly observing deforestaion rates across the landscape in each time period, the researcher observes annualized maps depicting pixel-level, binary deforestation ($y_{ivt}\in 0,1$). We follow [@kerr; @pfaff2004a] and model these binary deforestation events as a function of each pixel's unobservable value along a continous, latent variable ($y^*_{ivt}$) indicating the return to clearing pixel $i$, in property $v$, in year $t$. The latent variable is defined:
...
One can also use fixed effects at the level of an aggregated unit rather than the pixel level to resolve bias associated with pixel-level TWFE regressions with pixel fixed effects. Columns 6-8 of Figure \@ref(fig:summary-fig) show that pixel level TWFE regressions are unbiased estimators of the $ATT$ when grid, county, or property fixed effects are used rather than pixel fixed effects. We also see that, in the absence of property level perturbations (i.e. $\sigma_p=0$ in the DGP), all three models provide nearly identical estimates and estimate distributions. The RMSE of these models is slightly smaller than that of the corresponding models with aggregated units of analysis. 

<!-- ```{r agg_fe, fig.cap="fig caption", fig.show='hold', fig.pos = 'H'} -->
<!-- knitr::include_graphics("figs/aggregated_fe.png") -->
<!-- ``` -->

```{r summary-fig, fig.cap="bias, distribution, and coverage of all models with standard errors clustered at the level of the model fixed effects. The pixel level DID is clustered at the pixel level. ", fig.show='hold', fig.pos = 'H', out.width="100%"}
...
The introduction of property level unobservables changes the relative performance of each level of aggregation. Figure \@ref(fig:property-uoa) demonstrates that models incorporating the property into the analysis, whether with aggregated units of analysis or fixed effects, perform relatively better than their counterparts in terms of bias when the standard error governing the property level disturbances, $\sigma_p$, increases. The left panel of Figure \@ref(fig:property-uoa) demonstrates that the property-level specification (orange) suffers from less bias that models with any other unit of analysis when $\sigma_p$ is non-zero. The pixel level DID suffers from the highest levels of bias relative to alternative units of analysis. 
