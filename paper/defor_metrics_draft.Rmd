...
This generic clearing rule underpins a broad class of more specific static and dynamic models that have been used to explore the determinants of deforestation [e.g., @pfaff1999; @kerr; @pfaff2004].
...
## Estimating the $ATT$: difference-in-differences and two-way fixed effects estimators {#ATT}
...
DID and TWFE methods have become so popular in part, because the researcher does not need random assignment of treatment to generate convincing estimates of a program's impact on avoided deforestation. Instead, the researcher must make a common trends assumption, under which we evaluate each method. 
...
Often, however, the researcher wants to estimate the $ATT$ in a setting that does not fit the two-group, two-period case covered by DID models. In such cases, TWFE regressions are frequently used to apply DID methods to multiple groups or treatment periods. This amounts to estimating a regression that controls for unit and time fixed effects, which control for unobservable confounding variables that may vary across units or through time. 
...
In the case of two groups and two time periods, the TWFE regression typically gives an estimate equivalent to that of the DID model [@wooldridge]. With this in mind, many researchers, including those in the conservation impact evaluation literature, have used the TWFE model as a "generalized DID" that can be estimated in settings in which units undergo treatment in more than two distinct time periods (Table \@ref(tab:table-lit)). For example, a researcher may use a TWFE regression model to examine the effectiveness of a network of protected areas where the protected areas were created at different times, or a payment for ecosystem services (PES) program that enrolled properties in annual cohorts. 
...
Alternately, some researchers opt to keep deforested pixels in the panel, and choose to estimate an intervention's impact on deforested area, rather than on deforestation rates. However, when reforestation timing is unaccounted for, as is the case with most data products using a binary measure of deforestation, this is not possible. Rather than measuring deforested area at any given time, this outcome variable measures the stock of ever-deforested area through the current time period. It is unclear that this is a relevant parameter of interest in many cases and should not be interpreted as either a deforestation rate or the deforested area in a given time period.

## Estimating the $ATT$: survival analysis

While survival models have not frequently been used in the context of avoided deforestation, several other literatures utilize them in settings with irreversible outcomes of interest such as mortality (e.g., @friedman) or recidivism (e.g., @mastrobuoni). Here, the researcher is interested in the time each unit remains in the sample, which in our context, corresponds to the time each pixel survives until deforestation. While this class of models cannot estimate the $ATT$ directly, we present a hazard ratio analog that should be of similar interest:

$$ HRTT = \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=1]}$$

The $HRTT$ is the ratio of the hazard rates in the treatment group post-treatment, relative to the counterfactual in that group had treatment not ocurred. In settings with two groups and two time periods, some researchers opt to estimate $HRTT$ with a cox proportional hazards model of the following form:
...
, where $h(t)$ is the hazard rate of deforestation, $t$ years into the study period; and $\delta_0(t)$ is the baseline hazard function. Then, the expectation is that $exp(\beta_{coxDID}) = HRTT$.

### Proposing an alternative survival estimator

While models in the form of Regression 3 are an intuitive way to generalize the DID to survival analysis, $exp(\beta_{coxDID})$ generally does not recover the $HRTT$. Instead, it recovers a less intuitive product of pre- and post-treatment ratios across the two groups:
...
We propose an alternate estimator to recover the more relevant $HRTT$ parameter using three separate hazard rate ratios: 

\begin{align*}
HRR_1 &= \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t\geq t_0, D_i=0]}\\
HRR_2 &= \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1]} {E[y_{ivt}(0)  |  t < t_0, D_i=1]}\\
HRR_3 &= \frac{E[y_{ivt}(0)  |  t\geq t_0, D_i=0]} {E[y_{ivt}(0)  |  t < t_0, D_i=0]}
\end{align*}

Our estimator is then constructed by:
$$\widehat{HE} = \frac{1}{1/exp(\hat{ \alpha }) + 1/exp(\hat{ \beta }) - 1/(exp(\hat{ \beta })*exp(\hat{ \delta }))}$$

Each of $HRR_1$, $HRR_2$, and $HRR_3$ can easily be estimated individually by Cox regressions. Specifically,

$$h(t) =\lambda_0(t) exp(\alpha 1\{ t \geq t_0\} ),$$ subsetting to only the treated group;
$$h(t) =\gamma_0(t) exp( \beta 1\{ D_i = 1\} ),$$ subsetting to only the post period; and
$$h(t) =\psi_0(t) exp(\delta 1\{ t \geq t_0\} ),$$ subsetting to only the untreated group;

where $h(t)$ is the hazard rate of deforestation at time t.
...
Because the numerator of $HRTT$, $E[y_{ivt}(1)  |  t\geq t_0, D_i=1]$, is observable, we can recover the $ATT$ using the observable term and $HRTT$:

$$ATT = E[y_{ivt}(1)  |  t\geq t_0, D_i=1] - \frac{E[y_{ivt}(1)  |  t\geq t_0, D_i=1] }{HRTT}$$

Here, we have shown that the simple extension of the traditional DID to the survival setting does not clearly recover a parameter of interest to researchers interested in the impact evaluation. In contrast, our proposed estimator, which relies on separate estimation of relevant hazard rate ratios, does recover the relevant analog of the $ATT$. We explore the performance of $\hat{HE}$ relative to the proposed OLS regressions under various circumstances likely to arise in the deforestation setting in the next sections. 
...
\begin{align}
y^o_{ivt} = \alpha + \beta_{FE,j} \text{ x } D_i \mathbb{1} \{ t\geq t_0\} + \alpha_1 D_i + \lambda_t + \gamma_j + \epsilon_{it}
\end{align}
, where $\lambda_t$ denotes year fixed effects and $\gamma_j$ denotes one of grid ($g$), county ($c$), or property ($v$) fixed effects ($j=\{g, c, v \}$). 

Columns 3-5 of Figure \@ref(fig:summary-fig) show that pixel level TWFE regressions are unbiased estimators of the $ATT$ when grid, county, or property fixed effects are used rather than pixel fixed effects. We also see that, in the absence of property level perturbations (i.e. $\sigma_p=0$ in the DGP), all three models provide similar estimates and estimate distributions. 

```{r summary-fig, echo = FALSE, warnings = FALSE, results = FALSE, message=FALSE, fig.cap="bias, distribution, and coverage of all models with clustered standard errors.",  fig.width = 9, fig.height = 5.5, out.width="100%"}
...
df_summary <- specchart_long %>%
  filter(is.na(notes) & weights == 0 & (grid.fe == 0 | gridsize == 12)) %>%
...
df_summary <- rbind(df_summary[10,], df_summary[6,],  df_summary[5,], df_summary[1:3,], df_summary[7:9,], df_summary[4,])
...
labels <- list("Unit of analysis:" = c("pixel", "grid", "property", "county"),
               ...
               "Survival" = c("Cox PH", "HE estimator"),
...
schart(test,labels, ylim = ylim, index.ci=index.ci, col.est = c(palette$dark, palette$red)
       , ylab="Bias", highlight=c(1,2)
       ,band.ref=c(-.05, .04)
       , axes = FALSE
       #, col.band.ref="#c7e9f9"
...
Columns 6-8 of Figure \@ref(fig:summary-fig) show that the bias of the estimates does not vary dramatically across different levels of aggregation, however, the distributions are varied. In particular, the property-level model had the highest RMSE of unbiased specifications due to the heterogeneity in property sizes across the landscape. 

## Survival analysis: newly proposed estimator

In light of the fact that the simple Cox DID is not a viable analog to the traditional DID, we explore whether our proposed estimator, recovers relevant treatment effect parameters in the deforestation setting. We see in column 10 that $\widehat{HE}$ indeed recovers our parameter of interest.

One of the primary reasons for the use of survival analysis is censoring. This occurs when the researcher has partial information about the subjects’ survival times but does not have access to precise event times. While the researcher may not observe all pixels until they are deforested, many other forms of censoring are not a concern in the context of deforestation. Further, the proposed strategy to drop pixels in the periods after they are first deforested successfully addresses the irreversibility in deforestation events. As we show in the coming sections, survival analysis is likely to suffer from bias when unobservables enter the landscape at a scale different than the unit of analysis. While researchers can account for this in OLS specifications, there is no clear solution in survival analysis. 

# Non-random selection and the spatial nature of the deforestation process {#property}

<!-- Robert to do: Edit this section
-->
The irreversible nature of the deforestation process creates the potential for non-random sample selection in this data setting. Because the process of deforestation is irreversible in the panel, deforested pixels are no longer at risk of clearing in the periods after they are first deforested. This means the "at risk" set of pixels changes through time as more pixels become deforested. As such, the distribution that describes the returns to clearing the at-risk pixels may change through time as well, leading to non-random selection of the sample through time. For example, pixels with extremely high returns to clearing are more likely to be cleared early on, regardless of treatment status. In subsequent periods, therefore, these high return pixels are less likely to be present in the sample at all. In the context of two-groups and two-periods, only the second period suffers from this non-random sample selection, and we expresss the bias introduced from non-random sample selection below.
...
df_summary <- specchart_long %>%
  filter(is.na(notes) & pixel.fe == 0 & weights == 0 & (treatment.fe == 0 | se_pixel == 1) & (cox == 0 | HE.estimator ==1) & (grid.fe == 0 | gridsize == 12)) %>%
  mutate_at(vars(bias, cover), as.numeric
         )%>%
...
# 
# df_summary <- rbind(df_summary[5,], df_summary[7:9,], df_summary[1:3,], df_summary[6,], df_summary[4,])

par(oma=c(1,0,1,1))

labels <- list("Unit of analysis:" = c("pixel", "grid", "property", "county"),
               "Fixed effects:" = c("pixel FE", "grid FE", "property FE", "county FE", "treatment FE"),
               "Survival" = c("HE estimator"),
               "SE structure:" = c("clustered at pixel", "clustered at grid", "clustered at property", "clustered at county"))
...
lapply(1:length(RMSE), function(i) {
  # text(x= i, y=min(ylim)+.002, paste0(i), col="black", font=2, cex = 1)
  #mtext(paste0(i), side=1, at = i, font=2, cex=.95)#, line=1, at=-1)
  text(x= i, y=topline-0.0065, paste0(RMSE[i]), col="black", font=1, cex=1.1)
  text(x= i, y=midline - 0.007, paste0(coverage[i]), col="black", font=1, cex=1.1 )
})
# mtext('coverage\nprobability', side=2, las=1, at = loc_1, adj = 1, font=2, line=1)
# mtext('RMSE', side=2, las=1, at = loc_2, font=2, line=1)
text(x=mean(1:nrow(test))
     , y=midline-.0025, "coverage probability", col="black", font=2, cex = 1.25)
text(x=mean(1:nrow(test))
     , y=topline-.0025, "RMSE", col="black", font=2, cex = 1.25)
...
Connecting the econometric model to the process by which land use change occurs on the ground has clear benefits for estimation and inference in deforestation impact evaluation. Table \@ref(tab:table-lit) shows that researchers often use a binary point, pixel, or grid cell as the unit of analysis. While this may sometimes be the preferred unit, it can lead to biased results if the land use change process operates at a different scale.

Property level unobservables impact both treatment effect estimates and coverage probabilities. This is likely to be a factor when land use decisions are made at the property level, or the intervention seeks to alter underlying landowner incentives. Characteristics of land are also likely to be similar within the same spatial boundaries. In our Monte Carlo simulations, we introduce the standard deviation of the time-invariant property level disturbances in the initial DGP, $\sigma_p$, in order to account for these unobservables. 
...
```{r did-property, results = FALSE, message = FALSE, warning = FALSE, fig.align="center", fig.cap="bias, distribution, and coverage of pixel-level DID as property level disturbances play larger role in the data generating process", fig.width = 10, fig.height = 8, out.width = "75%", fig.pos='H', fig.show='hold'}
...
topline = -0.017
midline = topline-0.012
ylim <- c(midline-0.011,0.007)
...
schart(as.data.frame(schart_results), labels = labels, 
       ...
       heights = c(6,1), cex = c(1.75,1.75))

mtext("Bias     ", side=2, at = -0.00, font=2, las=1, line=.5, cex = 2)
Axis(side=2, at = c(-0.015, -0.01, -0.005, 0, 0.005, 0.01), labels=TRUE, cex = 4)
abline(h=topline)
abline(h=midline)
lapply(1:length(RMSE), function(i) {
  text(x= i, y=midline+0.006, paste0(RMSE_print[i]), col="black", font=1, cex=2)
  text(x= i, y=midline-0.006, paste0(c_print[i]), col="black", font=1, cex=2)
})
mtext("RMSE", side=2, at = midline+0.006, font=2, las=1, line=.5, cex = 2)
mtext("Coverage\nprobability", side=2, at = midline-0.006, font=2, las=1, line=.5, cex = 2)
mtext(paste0(0), side=1, at = 2, font=2, cex=2, line=3)
mtext(paste0(0.1), side=1, at = 3, font=2, cex=2, line=3)
mtext(paste0(0.25), side=1, at = 4, font=2, cex=2, line=3)
mtext(paste0(0.5), side=1, at = 5, font=2, cex=2, line=3)
mtext(bquote("Value of " ~ sigma[p]), side=1, at = 0, font=2, cex=2.5, line=3)

```

In Figure \@ref(fig:agg), we see that incorporating spatially aggregated units reduces bias relative to the simple pixel-level DID when the property influence is relatively large ($\sigma_p= 0.5$). This improvement is apparent in both specifications incorporating spatially aggregated units as the level of unit fixed effects (Regression 4; left panel) or as the unit of analysis (Regression 5; right panel). Further, we see that the scale of aggregation plays a role. Analyses incorporating the property or grid cells near the size of the average property outperform models utilizing larger or smaller scales. Ultimately, all models that incorporate spatial aggregation suffer from relatively little bias and incur less bias than the simple pixel-level DID. In Appendix \@ref(property-alt), we see that specifications incorporating the property tend to outperform other models in alternative parameterizations as well. 

<!-- ```{r agg-fe, results = FALSE, message = FALSE, warning = FALSE, fig.align="center", fig.cap="bias, distribution, and coverage with $\\sigma_p=0.5$ for Left: specifications with aggregated unit fixed effects; and Right: specifications with aggregatiod units of analysis", fig.width = 10, fig.height = 6, out.width = "75%", fig.pos='H', fig.show='hold'} -->
...
<!--        highlight = 5,  leftmargin = 14, col.est = c(palette$dark, palette$red), heights = c(3,1.5), cex = c(1.75, 1.75)) -->
...
<!-- schart(as.data.frame(schart_results), labels = labels, ylim = ylim, axes = FALSE, index.ci=index.ci, ylab="",  -->
<!--        highlight = 4,  leftmargin = 13, col.est = c(palette$dark, palette$red), heights = c(3, 1.5), cex = c(1.75, 1.75)) -->
...
```{r agg, results = FALSE, message = FALSE, warning = FALSE, fig.align="center", fig.cap="bias, distribution, and coverage with $\\sigma_p=0.5$ for Left: specifications with aggregated unit fixed effects; and Right: specifications with aggregated units of analysis", fig.width = 12, fig.height = 8, out.width = "100%", fig.pos='H', fig.show='hold'}

full_summary <- readRDS("results/summary_full.rds") %>%
  mutate_at(vars(bias, cover), as.numeric)%>%
  filter(std_p==0.5 & is.na(notes) & pixel.fe == 0 & weights == 0 & cox == 0 & (treatment.fe == 0 | se_pixel == 1) , is.na(gridsize) | gridsize == max(gridsize, na.rm = T)) %>%
...
select_results_A <- full_summary %>% 
  filter(pixel==1 ) %>% 
  arrange(desc(treatment.fe), desc(grid.fe), desc(county.fe), desc(property.fe))

select_results_B <- full_summary %>% 
  filter(pixel==0) %>% 
  arrange(desc(pixel), desc(treatment.fe), desc(grid.fe),  desc(county.fe), desc(property.fe))
...
schart_results <- select_results %>% 
 # mutate(F_col = FALSE)%>%
  select(c(Bias, q05, q95, pixel, county, property, grid, treatment.fe, county.fe, property.fe, grid.fe#,  
           #F_col
           ))%>%
  mutate_at(vars(pixel, grid, county, property, treatment.fe, grid.fe, county.fe, property.fe), ~as.logical(as.numeric(.)))

index.ci <- match(c("q05","q95"), names(schart_results))

labels <- list("Unit of analysis:" = c("pixel","county", "property", "grid"),
               "Fixed effects:" = c("treatment FE", "county FE", "property FE", "grid FE"),
               "Grid size" = c("")
               )
...
schart(as.data.frame(schart_results), labels = labels, ylim = ylim, axes = FALSE, index.ci=index.ci, ylab="", 
       highlight = c(5, 9),  leftmargin = 16, col.est = c(palette$dark, palette$red), heights = c(3, 3), cex = c(1.75, 1.75)
       )
mtext("Bias    ", side=2, at = 0.0, font=2, las=1, line=.5, cex = 2)
Axis(side=2, at = c(-0.01, 0, 0.01), labels=TRUE, cex = 3)
abline(h=topline)
abline(h=midline)
lapply(1:length(RMSE), function(i) {
  text(x= i, y=midline+0.006, paste0(RMSE_print[i], " "), col="black", font=1, cex=1.5)
  text(x= i, y=midline-0.006, paste0(c_print[i]), col="black", font=1, cex=1.5)
#  mtext(gridsize_print[i], side=1, at = i, font=2, cex=1.5, line=16)
})
mtext("RMSE", side=2, at = midline+0.006, font=2, las=1, line=.5, cex = 1.75)
mtext("Coverage\nprobability", side=2, at = midline-0.006, font=2, las=1, line=.5, cex = 1.6)
abline(v = 6)


```

While incorporating spatially aggregated units in OLS specifications may improve analysis, there is no clear analog with survival models. In fact, Figure \@ref(fig:summary) in the appendix shows that $\hat{HE}$ suffers significantly as $sigma_p$ increases. As such, when the pixel is not the preferred unit of analysis, survival analysis is not likely to be the preferred method for estimating the impact of conservation interventions.
...
summary <- summary_pweights %>%
  mutate_at(vars(estimate, cover, given_ATT, p_ATT), as.numeric
  )%>%
  ...
  filter(is.na(notes) & (county.fe == T | property.fe == T) )%>%
  mutate(bias = estimate - given_ATT,
         ATT_diff = p_ATT - given_ATT)%>%
  group_by(pixel, property, county, property.fe, county.fe, weights, se_property, se_county)%>%
...
df_summary <- summary %>%
  select(-c( ATT, p_ATT, ATT_diff))

par(oma=c(1,0,1,1))

labels <- list("Unit of analysis:" = c("pixel", "property", "county"),
               "Fixed effects:" = c("property FE", "county FE"),
               "Weights:" = c("area weights"),
               "SE structure:" = c("clustered at property", "clustered at county"))
coverage <- round(df_summary$cover, digits=3)
RMSE <- round(df_summary$RMSE, digits=5)
test <- as.data.frame(subset(df_summary, select=-c(cover, RMSE)))

test <- subset(test, select=-c(se_property, se_county))%>%
  distinct()
...
abline(h=mean(summary$ATT_diff), col = palette$red, lty = "dashed")
...
my_event_study_plot(county_es, seperate = FALSE)+
  ggtitle("Estimates with aggregated unit of analysis")+
  ylim(-0.045, 0.045)
...
full_summary <- summary_full %>%
  ...
  filter(is.na(notes) & pixel.fe == FALSE & (treatment.fe == 0 | se_pixel == TRUE) & (grid.fe == 0 | gridsize == 12))%>%
...
text(x=2
     , y=.017, expression(paste(sigma[p],"=0.0")), col="black", font=2, cex = 1.2)
text(x=14
     , y=.017, expression(paste(sigma[p],"=0.1")), col="black", font=2, cex = 1.2)
text(x=26
     , y=.017, expression(paste(sigma[p],"=0.2")), col="black", font=2, cex = 1.2)
text(x=38
     , y=.017, expression(paste(sigma[p],"=0.3")), col="black", font=2, cex = 1.2)
...
df_summary <- readRDS("results/summary_long_alt.rds") %>%
  ...
  filter(is.na(notes) & pixel.fe == FALSE & (treatment.fe ==0 | se_pixel == 1))%>%
...
text(x=mean(1:nrow(test))
     , y=topline-.005, "RMSE", col="black", font=2)
legend(x=3.5, y=0.055, col = palette$red, legend = "specifications incorporating property", seg.len=0.65, inset = 0.005,  box.lty=0, cex=0.8, lty = 1, lwd = 4, bg="transparent")
...
text(x=mean(1:nrow(test))
     , y=topline-.005, "RMSE", col="black", font=2)
legend(x=3.5, y=0.025, col = palette$red, legend = "specifications incorporating property", seg.len=0.65, inset = 0.005,  box.lty=0, cex=0.8, lty = 1, lwd = 4, bg="transparent")
...
```{r deforrates, message=FALSE,out.width="75%", fig.cap="Distribution of estimates produced by different outcome variable formulae", fig.pos='H', fig.align='center'}
...
ggplot(data = cbias, aes(x = bias, fill=variable)) +
    ...
    theme_minimal(base_size = 22)+
  scale_fill_manual(labels=c("Outcome 1", "Outcome 2", "Outcome 3"), values = c(palette$red, palette$blue, palette$green)
                    )
```
