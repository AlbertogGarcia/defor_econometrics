...
The $HRTT$ is the ratio of the hazard rates in the treatment group post-treatment, relative to the counterfactual in that group had treatment not ocurred. In settings with two groups and two time periods, some researchers opt to estimate $HRTT$ with a cox proportional hazards model of the following form [e.g., @carlson2018; @mastrobuoni]:
...
Table \@ref(tab:res) helps us to get a sense of relative scale within our synthetic landscapes for this section. We assume pixels are comparable to Landsat resolution (30 m).
...
kable(resolution_df, format = "latex", row.names = FALSE,  booktabs = T,
       caption = "Spatial unit resolution and comparable area",
       col.names = NULL) %>%
  add_header_above(c("Spatial unit"=1,"Avg. number of pixels"=1, "Area (hectares)"=1), escape = TRUE, line =TRUE, bold = TRUE)%>%
  kable_styling(font_size = 10, latex_options = c("HOLD_position", "striped"),
                position = "center")

```

In Figure \@ref(fig:agg), we see that incorporating spatially aggregated units reduces bias relative to the simple pixel-level DID when the property influence is relatively large ($\sigma_p= 0.5$). This improvement is apparent in both specifications incorporating spatially aggregated units as the level of unit fixed effects (Regression 4; left panel) or as the unit of analysis (Regression 5; right panel). That said, specifications with a spatially aggregated unit of analysis consistently outperform their counterpart with spatially aggregated fixed effects. Further, we see that the scale of aggregation plays a role. Analyses incorporating the property or grid cells near the size of the average property outperform models utilizing larger or smaller scales. Ultimately, all models that incorporate spatial aggregation suffer from relatively little bias and incur less bias than the simple pixel-level DID. In Appendix \@ref(property-alt), we see that specifications incorporating the property as the unit of analysis tend to outperform other models in alternative parameterizations as well. 
...
full_summary <- readRDS("results/summary_full.rds") %>%
  mutate_at(vars(bias, cover), as.numeric)%>%
  filter(std_p==0.5 & is.na(notes) & pixel.fe == 0 & weights == 0 & cox == 0 & (treatment.fe == 0 | se_pixel == 1) , is.na(gridsize) | gridsize != min(gridsize, na.rm = T)) %>%
...
schart(as.data.frame(schart_results), labels = labels, ylim = ylim, axes = FALSE, index.ci=index.ci, ylab="", 
       highlight = c(6, 11),  leftmargin = 16, col.est = c(palette$dark, palette$red), heights = c(3, 3), cex = c(1.75, 1.75)
       )
...
text(x=23
     , y=.017, expression(paste(sigma[p],"=0.25")), col="black", font=2, cex = 1.2)
text(x=33
     , y=.017, expression(paste(sigma[p],"=0.5")), col="black", font=2, cex = 1.2)
