...
y_nogrouptrend <- quickmontey(100, 10000, 3, .05, 0, 0, -.16)
y_nogrouptrend
```
...
y_notrend <- quickmontey(100, 10000, 3, .05, -.1, 0, -.16)
y_notrend
...
y_nogroup <- quickmontey(100, 10000, 3, .05, 0, 0.1, -.16)
y_nogroup
...
ypos <- quickmontey(100, 10000, 3, .05, -0.25, 0.2, .16)
yneg <- quickmontey(100, 10000, 3, .05, -0.25, 0.2, -.16)
ypos
yneg
...
There may be a tiny bit of bias introduced by adding in the individual level errors here. The magnitude of this bias is extremely small, however.
...
Reducing the standard error of the individual level error term seems to remove that bias. 
...
```{r message=FALSE, results='hide'}
yit_nogrouptrend <- quickmonteyit(100, 10000, 3, -.15, 0, 0, -.16)
yit_nogrouptrend
```

There is no bias with only the intercept and treatment effect in the model. Next, we introduce the group effect. 


```{r message=FALSE, results='hide'}
yit_nogroup <- quickmonteyit(100, 10000, 3, -.15, 0.1, 0, -.16)
yit_nogroup
```

There is no bias with only the group effect. Now, instead of the group effect, we introduce the time effect. 



```{r message=FALSE, results='hide'}
yit_notrend <- quickmonteyit(100, 10000, 3, -.2, 0, .1, -.15)
yit_notrend
```

Here, there is still no bias with a time effect introduced but no group effect. 
...
Again, this is the point where the bias enters. It seems that the bias is positive when the time and group effects have the same sign and negative when the signs differ. This is regardless of the sign of the treatment effect. 
