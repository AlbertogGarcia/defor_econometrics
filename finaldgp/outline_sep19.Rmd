...
It seems that aggregating to the properties vs grids here makes a smaller difference with this set of parameters. 
...
This function includes three possible aggregation methods: county, property, and grid level. We then compute the coverage propbability of the ATT with a 95% CI, clustering standard errors at the group level. 
...
The property level aggregation leads to the least bias in this case. It also results in the expected coverage, while the county and grid level aggregation results in under coverage. It is unclear how much of the issue is related to the bias and not the standard errors themselves. 
...
Weighting the regressions by area does seem to generate slightly more bias compared to the ATT, however, this is likely because the estimates are less biased relative to the estimand. 