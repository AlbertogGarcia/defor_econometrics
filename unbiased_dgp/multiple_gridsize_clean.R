...
multiple_gridsize_clean <- function(n, nobs, years, b0, b1, b2_0, b2_1, b3, std_a = 0.1, std_v = 0.25, std_p = 0.0, cellsize_list, ppoints, cpoints, rm.selection = FALSE){
  ...
  summary_long <- data.frame('b0'= rep(b0, summ_row), 'b1'= rep(b1, summ_row), 'b2_0'= rep(b2_0, summ_row), 'b2_1'= rep(b2_1, summ_row),  
                             ...
                             'iteration' = rep(NA, summ_row), 'avg_garea' = rep(NA, summ_row), "property" = rep(NA, summ_row),
  ...
  for(i in 1:n){
    ...
    for(k in cellsize_list){
      ...
      # bias
      grid_coeffmatrix[i, pos] <- DID$coefficients - ATT
      
      #clustering at grid level
      clse    <- tail(summary(DID, cluster = ~this_grid)$se, n=1)
      
      grid_covermat[i,pos] <- dplyr::between(ATT, tail(DID$coefficients, n=1) - 1.96 * clse, tail(DID$coefficients, n=1) + 1.96 * clse)*1
      ...
      # bias
      pix_coeffmatrix[i,pos] <- tail(pix_twfe$coefficients, n=1) - ATT
      
      #clustering at grid level
      clse    <- tail(summary(pix_twfe, cluster = ~this_grid)$se, n=1)
      
      pix_covermat[i,pos] <- dplyr::between(ATT, tail(pix_twfe$coefficients, n=1) - 1.96 * clse, tail(pix_twfe$coefficients, n=1) + 1.96 * clse)*1
      ...
      summary_long[i+((pos-1)*n),c(firstcol:lastcol)] <- c(
        0,1,
        i,
        mean(this_grid_df$this_garea),
        NA,
        grid_coeffmatrix[i,pos],
        grid_covermat[i,pos]
      )
      
      summary_long[i+((pos-1)*n)+(summ_row/2),c(firstcol:lastcol)] <- c(
        1,0,
        i,
        mean(this_grid_df$this_garea), 
        NA,
        pix_coeffmatrix[i,pos],
        pix_covermat[i,pos]
      )
    ...
    property_df <- as.data.frame(panels) %>%
    ...
    property_df <- property_df[order(property_df$property, property_df$year),]
    property_df <- slide(property_df, Var = "defor", GroupVar = "property", NewVar = "deforlag",
                         slideBy = -1, reminder = FALSE)
    
    ##### creating forested share variable #####
    property_df$forshare <- 1 -  property_df$defor
    property_df$forsharelag <- 1 -  property_df$deforlag
    
    #generate outcome var
    property_df$deforrate <- ((property_df$forsharelag- property_df$forshare) / property_df$forsharelag)
    #remove any infinite values
    #gridlevel_df <- subset(gridlevel_df, select = -c(geometry))
    property_df <- 
      property_df %>% 
    ...
    # aggregated units of analysis
    DID <- feols(deforrate ~  post*treat|year+property, data = property_df)
    
    # bias
    p_agg <- DID$coefficients - ATT
    
    #clustering at grid level
    clse    <- tail(summary(DID, cluster = ~property)$se, n=1)
    
    p_agg_cover <- dplyr::between(ATT, tail(DID$coefficients, n=1) - 1.96 * clse, tail(DID$coefficients, n=1) + 1.96 * clse)*1
    ...
    # aggregated units of analysis
    pix_twfe <- feols(y_it ~  post*treat|year+property, data = panels)
    
    # bias
    p_pix <- tail(pix_twfe$coefficients, n=1) - ATT
    
    #clustering at grid level
    clse    <- tail(summary(pix_twfe, cluster = ~property)$se, n=1)
    
    p_pix_cover <- dplyr::between(ATT, tail(pix_twfe$coefficients, n=1) - 1.96 * clse, tail(pix_twfe$coefficients, n=1) + 1.96 * clse)*1
    
    summary_long[n_mod*n + i,c(firstcol:lastcol)] <- c(
      0, 0,
      i,
      NA, 
      "unit of analysis",
    ...
    summary_long[n_mod*n + n + i,c(firstcol:lastcol)] <- c(
      0, 0,
      i,
      NA, 
      "fixed effects",
