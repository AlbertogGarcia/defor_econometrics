...
#begin function
multi_gridsize <- function(n, nobs, years, b0, b1, b2_0, b2_1, std_a = 0.1, std_v = 0.25, std_p = 0.0, std_b3 = 0.0, given_ATT, cellsize_list, ppoints, cpoints, rm.selection = FALSE){
  ...
  summary_long <- data.frame('b0'= rep(b0, summ_row), 'b1'= rep(b1, summ_row), 'b2_0'= rep(b2_0, summ_row), 'b2_1'= rep(b2_1, summ_row), 'std_b3'= rep(std_b3, summ_row), 
                             ...
                             'grid'=rep(1, summ_row),'grid fe'=rep(1, summ_row), 'parea' = rep(mean(prop_area$parea), summ_row),
  ...
  for(i in 1:n){
    ...
    for(k in 1:length(cellsize_list)){
      ...
      this_panel <- panels %>%
        ...
        select(this_grid, treat, post, year, defor, this_garea)
