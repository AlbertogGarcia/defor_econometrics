...
vary_outcome_yr <- function(n, nobs, years_min, years_max, ATT, trend, base_0, base_1, std_a = 0.1, std_v = 0.25, cellsize){
  
  year_range <- seq(from = years_min, to = years_max, by = 1)
  year_list <- as.list(year_range)
  
  #preallocate n x ___ matrix
  out_matrix1 <- matrix(nrow = n, ncol = length(year_list))
  out_matrix2 <- matrix(nrow = n, ncol = length(year_list))
  out_matrix3 <- matrix(nrow = n, ncol = length(year_list))
  ...
  outbias_df1 <- as.data.frame(cbind(colMeans(out_matrix1), year_range)) 
  outbias_df2 <- as.data.frame(cbind(colMeans(out_matrix2), year_range)) 
  outbias_df3 <- as.data.frame(cbind(colMeans(out_matrix3), year_range)) 
  ...
  plot <- ggplot() + 
    geom_line(data = outbias_df1, aes(x = year_range, y = V1, color="outcome 1"), size =1) +
    geom_line(data = outbias_df2, aes(x = year_range, y = V1, color="outcome 2"), size =1) +
    geom_line(data = outbias_df3, aes(x = year_range, y = V1, color="outcome 3"), size =1) +
    scale_color_manual(values = colors)+
    labs(x = "Years pre and post treatment in the observation period", y = "Bias", caption = paste("Bias as a function of observation period depends on deforestation formula")) + 
