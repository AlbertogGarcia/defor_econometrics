...
#begin function
functionalform_agg <- function(n, nobs, years, b0, b1, b2_0, b2_1, b3, std_a = 0.1, std_v = 0.25, cellsize){
  ...
  plot <- ggplot(data = cbias, aes(x = bias, fill=variable)) +
    ...
    scale_fill_discrete(breaks=c("outcome1", "outcome2"), labels=c("Grid Level Linear Model", "Grid Level Poisson"))+
    geom_vline(xintercept = 0, linetype = "dashed")+
    theme(plot.caption = element_text(hjust = 0.5))+
    labs(x= "Bias", caption = paste("Grid Level Linear Model Mean:", round(mean(coeff_bias$outcome1), digits = 4),
                                    ", RMSE:", round(rmse(actual, coeff_bias$outcome1), digits = 4), "\n", 
                                    "Grid Level Poisson Mean:", round(mean(coeff_bias$outcome2), digits = 4),
