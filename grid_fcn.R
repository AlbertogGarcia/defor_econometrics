...
# aggregate up to county in each year 
  suppressWarnings(
  countylevel_df <-  aggregate(df_match, by = list(df_match$county, df_match$treat, df_match$year), FUN = mean, drop = TRUE)[c("county", "treat", "post", "year","defor")]
  )
  gridlevel_df <- unite(countylevel_df, countyid, treat, county, sep = "_", remove = FALSE)
  assign('gridlevel_df',gridlevel_df, envir=.GlobalEnv)
